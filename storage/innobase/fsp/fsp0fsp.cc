...
/**********************************************************************//**
读取page + FSP_HEADER_OFFSET + FSP_SIZE 的值
	...
	mtr_t*	mtr)	/*!< in/out: mini-transaction */
{
	...
	byte_index = index / 8; //所在字节
	bit_index = index % 8;  //所在字节中的位偏移
	...
	mtr_t*		mtr)		/*!< in/out: mini-transaction */
{
	...
	/* 根据page number 得到 簇页的page number编号 */
...
#ifndef UNIV_HOTBACKUP
/**********************************************************************//**
数据文件中的第一个page类型就是FIL_PAGE_TYPE_FSP_HDR，
该page同时用于跟踪随后的256个Extent(约256MB文件大小)的空间管理
所以每隔256MB就要创建一个类似的数据页，类型为FIL_PAGE_TYPE_XDES ，
XDES Page除了文件头部外，其他都和FSP_HDR页具有相同的数据结构，
可以称之为Extent描述页，每个Extent占用40个字节，
一个XDES Page最多描述256个Extent。


初始化FIL_PAGE_TYPE_FSP_HDR类型的page
...
/*============*/
	ulint	space,		/*!< in: space id */
	ulint	size,		/*!< in: current size in blocks,page数量 */
	mtr_t*	mtr)		/*!< in/out: mini-transaction */
{
	fsp_header_t*	header;
	buf_block_t*	block;
	...
	const page_t*	page)	/*!< in: first page of a tablespace */
{
	...
	//fsp header中的 sapce_id
	fsp_id = mach_read_from_4(FSP_HEADER_OFFSET + page + FSP_SPACE_ID);

	//page header中的space_id
...
文件的扩展遵循一定的规则：如果当前小于1个Extent，则扩展到1个Extent满；
当表空间小于32MB时，每次扩展一个Extent；
大于32MB时，每次扩展4个Extent（fsp_get_pages_to_extend_ibd）。
	...
	mtr_t*		mtr)		/*!< in/out: mini-transaction */
{
	...
	*actual_increase = 0;
	...
	if (space == 0) {
		...
		ulint	extent_size;	/*!< one megabyte  1M, in pages */
		...
		if (!zip_size) {
			...
			extent_size = FSP_EXTENT_SIZE;// 默认1M空间，1个extent
		} else {
...
/**********************************************************************//**
申请一个extent,放到fsp_header的FSP_FREE链表中去
...
/*============*/
	...
	mtr_t*	init_mtr)	/*!< in: mini-transaction for initializing
				the page */
{
	/*buf_pool->free_list中获得一个buf_block对象 */
	buf_block_t*	block
		= buf_page_create(space, page_no, zip_size, init_mtr);
	printf("fsp_page_create_dbg: space id = %d  page_no=%d \n", space, page_no);
	...
	/*增加block->page.buf_fix_count，block->page.buf_fix_count++ */
	buf_block_buf_fix_inc(block, __FILE__, __LINE__);
	mutex_exit(&block->mutex);

	mtr_memo_push(init_mtr, block, MTR_MEMO_PAGE_X_FIX);

	/*page_block的分配和初始化同时完成 */
	if (init_mtr == mtr
...
/**********************************************************************//**
从space header 的 FSP_FREE_FRAG中获取free page
...
/**********************************************************************//**
标记管理page的相应的xdes，将page加入到FSP_FREE_FRAG中去
...
/**********************************************************************//**
分配一个inode page,FIL_PAGE_TYPE = FIL_PAGE_INODE
并且添加到space_header + FSP_SEG_INODES_FREE
	...
	mtr_t*		mtr)		/*!< in/out: mini-transaction */
{
	...
	*/
	...
	/*
	   page type 为 FIL_PAGE_INODE
	*/
	mlog_write_ulint(page + FIL_PAGE_TYPE, FIL_PAGE_INODE,
			 MLOG_2BYTES, mtr);

	/*
	  将inode中的FSEG_ID全部设置为0，都是free状态的
	*/
	...
	/*
	  本page 插入到 space_header + FSP_SEG_INODES_FREE链表中去
	  将 FSEG_INODE_PAGE_NODE作为 free inode 插入到space header中去 
	*/
	flst_add_last(
...
/**********************************************************************//**
调用fsp_seg_inode_page_find_free查找一个空闲的inode
	...
	mtr_t*		mtr)		/*!< in/out: mini-transaction */
{
	...
	//首先从file space header中的 inode_free链表中寻找空闲的inode
	if (flst_get_len(space_header + FSP_SEG_INODES_FREE, mtr) == 0) {
		/* Allocate a new segment inode page */
		/*
		  如果 file space header 中找不到空闲的inode了，然后就要分配一个inode page(FIL_PAGE_INODE)了，
		  并且将这个页面中的inode都放到space_header + FSP_SEG_INODES_FREE链表中去
		 */
...
得到inode->FSEG_FRAG_ARR[n]的 page number
...
/**********************************************************************//**
将page_no写入inode的第n个fragment page
...
/**********************************************************************//**
从inode的FRAG_ARR数组中查找到一个空闲的 page slot
...
/**********************************************************************//**
inode 中 的32个page number不为 FIL_NULL的数量
...
创建Segment,初始化segment inode 以及segment header,
函数变量page表示 segment header的位置，
如果page 等于0，则表示创建的segment header在创建的段(segment)中,更准确的说是在段的碎片页中(32个fragment page中的一个)，
例如insert buffer 的方式


创建Segment 首先每个Segment需要从ibd文件中预留一定的空间(fsp_reserve_free_extents)，通常是2个Extent。
但如果是新创建的表空间，且当前的文件小于1个Extent时，则只分配2个Page。
...
/*================*/
	ulint	space,	/*!< in: space id */
	ulint	page,	/*!< segment header所在的位置 (page_no),
					in: page where the segment header is placed: if
			this is != 0, the page must belong to another segment,
			if this is 0, a new page will be allocated and it
			will belong to the created segment */
	ulint	byte_offset, /*!< 
						 segment header所在的page的内部偏移量
						 ，将segment 的inode信息写在此处
	...
	mtr_t*	mtr)	/*!< in/out: mini-transaction */
{
	...
	zip_size = fsp_flags_get_zip_size(flags);//通常是0

	/*
	  如果page!=0,则说明外面调用时，需要将段头信息放入到所传入的页面中(在另外的segment中)，

	  而对应的byte_offset参数就是要存储在这个页面中的偏移量(FSEG HEADER处)，
	  而如果page==0，则说明是要创建一个独立的段，并且将这个段本身的段头信息
	  存储到自己的第一个页面中去
	*/
	if (page != 0) {
	...
	/* 得到相应的space header,跳过 file space header page 的开头38个字节 */
	space_header = fsp_get_space_header(space, zip_size, mtr);


	/*
	  从得到的space heaer中分配Inode Entry,
	  一个只有85个inode，不会没有？？？
	*/
	...
	给新分配的inode指定seg_id号，这个ID号要从表空间头的FSP_SEG_ID中取出来作为新段的ID号，
	然后将这个ID号写入到INODE的FSEG_ID中，同时更新FSP_SEG_ID的值为：老值+1
	*/

	seg_id = mach_read_from_8(space_header + FSP_SEG_ID);
	/*space header中的 seg id增加1，给下次用*/
	...
	*/
	...
	for (i = 0; i < FSEG_FRAG_ARR_N_SLOTS; i++) {
		//设置 inode  slots[i](FSEG_FRAG_ARR)的page number 为NULL
		fseg_set_nth_frag_page_no(inode, i, FIL_NULL, mtr);
	}

	if (page == 0) {
		/*
		  page == 0,FILE SPACE HEADER pager
		*/
	...
	/* FSEG_HDR_OFFSET: byte offset of the inode */
	mlog_write_ulint(header + FSEG_HDR_OFFSET,
			 page_offset(inode), MLOG_2BYTES, mtr);

	/* FSEG_HDR_PAGE_NO: page number of the inode */
	mlog_write_ulint(header + FSEG_HDR_PAGE_NO,
			 page_get_page_no(page_align(inode)),
			 MLOG_4BYTES, mtr);

	/* FSEG_HDR_SPACE: space id of the inode */
...
/*========*/
	...
	ulint	page,	/*!< in: page where the segment header is placed: if
			...
			if this is 0, a new page will be allocated and it
			will belong to the created segment */
	ulint	byte_offset, /*!< in:在byte_offset处写入对应inode相关的信息，
						         byte offset of the created segment header
			                     on the page */
	mtr_t*	mtr)	/*!< in/out: mini-transaction */
{
	...
	mtr_t*		mtr)	/*!< in/out: mini-transaction */
{
	...
	/* inode 为在inode page中对应的inode偏移 */
	inode = fseg_inode_get(header, space, zip_size, mtr);

	/* inode管理的segment中已经使用的page */
	...
	mtr_t*		mtr)	/*!< in/out: mini-transaction */
{
	...
	for (i = 0; i < FSEG_FREE_LIST_MAX_LEN; i++) {
		...
		/*  XDES归属到某个segment，id=seg_id  */
		xdes_set_state(descr, XDES_FSEG, mtr);

		seg_id = mach_read_from_8(inode + FSEG_ID);
		ut_ad(mach_read_from_4(inode + FSEG_MAGIC_N)
		      == FSEG_MAGIC_N_VALUE);
		/* 设置xdes所属的 segment id */
...
crash:
	...
	if (state != XDES_FSEG) {
		...
		/* 从inode的 fragment page中查找相依的page_no */
		for (i = 0;; i++) {
