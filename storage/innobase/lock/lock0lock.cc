...
explicit lock 影响到record和record之前的records,有gap
implicit x-lock 没有gap，只是影响到record

修改cluster index之后，在cluster index 和 secondary index上的记录上都持有implicit x-lock，
在secondary index 上没有explicit non-gap lock

对记录进行插入或者修改的时候，会对记录持有implicit x-lock，在secondary index record，也是这个样子。
...
/*********************************************************************//**
获取lock->un_member.rec_lock的bitmap对应的位值
...
/*********************************************************************//**
在console打印出来 trx_id对应的trx是不正常的
...
/*********************************************************************//**
确认一个辅助索引记录是否可以在一个consistent read中可见
...
/*********************************************************************//**
只在innobase_start_or_create_for_mysql中调用一次
创建lock_sys对象，都是在内存中操作，没有写到磁盘上去
...
/*********************************************************************//**
获取alter table事物中的中的表,表必须已经被IS或者IX锁住了
...
/*********************************************************************//**
设置trx->lock.wait_lock,并且让lock进入LOCK_WAIT状态
...
/*********************************************************************//**
是否是LOCK_INSERT_INTENTION,
通过lock->type_mode & LOCK_INSERT_INTENTION得到结果
...
/*=================*/
	const trx_t*	trx,	/*!< 请求锁的事务  in: trx of new lock */
	ulint		type_mode,/*!< 请求的锁模式 in: precise mode of the new lock
				to set: LOCK_S or LOCK_X, possibly
				ORed to LOCK_GAP or LOCK_REC_NOT_GAP,
				LOCK_INSERT_INTENTION */
	const lock_t*	lock2,	/*!< 已经在某个record上锁住了的锁 in: another record lock; NOTE that
				...
				locks are record locks */
{
	...
	if (lock1->trx != lock2->trx /* 不是同一个事务 */
	    && !lock_mode_compatible(lock_get_mode(lock1),
				     lock_get_mode(lock2)) /* lock1和lock2不兼容 */ ) {
		...
		if (lock_get_type_low(lock1) == LOCK_REC) {
			...
			return(lock_rec_has_to_wait(lock1->trx,
						    ...
						    lock_rec_get_nth_bit(
							    lock1, 1)/* supermum记录是否锁住 */ ));
		}

		/* 表锁，不是同一个事务，不兼容，那就需要wait了 */
		return(TRUE);
	}
...
/*********************************************************************//**
返回lock->rec_lock->n_bits,bitmap的bits数量
...
/**********************************************************************//**
设置lock->rec_lock的bitmap的第i个位置，就是锁住第i条记录了
...
/**********************************************************************//**
返回lock->rec_lock的bitmap的首个被set的位置(即锁住的)
...
/*********************************************************************//**
在lock->hash中查找lock->un_member.rec_lock.space 和lock->un_member.rec_lock.page_no相等的lock对象
注意：不同的事务对同一个表拥有S_LOCK的情况
...
/*********************************************************************//**
调用lock_rec_get_next_on_page_const，作用一样
Gets the first or next record lock on a page.
@return	next lock, NULL if none exists */
UNIV_INLINE
lock_t*
	...
	ulint	page_no)/*!< in: page number */
{
	...
	for (lock = static_cast<lock_t*>(
	      ...
	      lock != NULL;
	      lock = static_cast<lock_t*>(HASH_GET_NEXT(hash, lock)/* 就是 lock=lock->hash */ )) {
	...
	ulint	page_no)/*!< in: page number */
{
	...
	lock_mutex_enter();
	/* 从lock_sys->lock_rec中得到第一个符合space,page_no的lock */
...
/*********************************************************************//**
lock_sys->rec_hash中对应的链表上的第一条符合 block的page no ,space id的lock_t对象
	...
	const buf_block_t*	block)	/*!< in: buffer block */
{
	...
	/*  就是block->lock_hash_val */
	hash = buf_block_get_lock_hash_val(block);

	for (lock = static_cast<lock_t*>(
			HASH_GET_FIRST( lock_sys->rec_hash, hash)); /* 取得相应的链表,lock_sys->rec_hash->array[hash]->node  */
	     lock != NULL;
	     lock = static_cast<lock_t*>(HASH_GET_NEXT(hash, lock) /* 就是lock=lock->hash->node */ )) {
	...
	lock_t*	lock)	/*!< in: lock */
{
	...
	} while (lock && !lock_rec_get_nth_bit(lock, heap_no) /* 第heap_no条记录没有lock */ );

	return(lock);
}

/*********************************************************************//**
获取第heap_no条记录锁住的page的lock
Gets the next explicit lock request on a record.
@return	next lock, NULL if none exists or if heap_no == ULINT_UNDEFINED */
UNIV_INLINE
const lock_t*
	...
	ulint			heap_no)/*!< in: heap number of the record */
{
	...
	/* 根据block的space_id 和 page_no得到 lock_t对象*/
	for (lock = lock_rec_get_first_on_page(block); lock;
	     lock = lock_rec_get_next_on_page(lock)) {

		/* lock->rec_lock的bitmap中第heap_no位是否设置为1 */
...
/*********************************************************************//**
in_lock负责的page的第heap_no条记录的前面的一条被锁的记录的那个锁
	...
	ulint		heap_no)/*!< in: heap number of the record */
{
	...
	/* 就是lock=lock_sys->rec_hash->array[fold].node */
	for (lock = lock_rec_get_first_on_page_addr(space, page_no);
	     /* No op */;
	     lock = lock_rec_get_next_on_page(lock) /* 就是 lock->hash */ ) {
			 
		ut_ad(lock);

		if (lock == in_lock) {
            /* 找到,如果是首个，那found_lock==NULL了 */
			return(found_lock);
		}
...
/*********************************************************************//**
确认事务是否已经对record有了更强或者相同等级的 explicit锁了
trx==lock->trx
	...
	const trx_t*		trx)	/*!< in: transaction */
{
	...
	for (lock = lock_rec_get_first(block, heap_no);
		...
		if (lock->trx == trx
		    && !lock_rec_get_insert_intention(lock)/* 是否是意向锁 */
		    && lock_mode_stronger_or_eq(
			    lock_get_mode(lock),
			    static_cast<enum lock_mode>(
				    precise_mode & LOCK_MODE_MASK)) /* 根据lock_strength_matrix矩阵判断，是否请求更强的锁 */
		    && !lock_get_wait(lock) /* 没有等待，LOCK_WAIT 位为0 */
		    && (!lock_rec_get_rec_not_gap(lock)
			|| (precise_mode & LOCK_REC_NOT_GAP)
			|| heap_no == PAGE_HEAP_NO_SUPREMUM) /* LOCK_REC_NOT_GAP 位为0。（没有这个标记默认就是 NEXT KEY LOCK，锁住行前面的gap） 或者 要加锁的 LOCK_REC_NOT_GAP 位为 1 或者 当前行为 PAGE_HEAD_NO_SUPREMUM, 表示上界 */
		    && (!lock_rec_get_gap(lock)
			|| (precise_mode & LOCK_GAP)
			|| heap_no == PAGE_HEAP_NO_SUPREMUM) /* LOCK_GAP 位为0 或者 要加锁的 LOCK_GAP 为 1 或者 当前行为 PAGE_HEAD_NO_SUPREMUM, 表示上界 */
			) {
					...
					are taken into account */
{
	...
	/* lock_sys->rec_hash->array[fold].node */
	for (lock = lock_rec_get_first(block, heap_no);
	     lock != NULL;
	     lock = lock_rec_get_next_const(heap_no, lock)) {

		if (lock->trx != trx /* 事务不一样 */
		    && (gap  /* 请求gap mode锁 */
			     || !(lock_rec_get_gap(lock) /* 已有的lock不是gap mode锁 */
			     || heap_no == PAGE_HEAP_NO_SUPREMUM /* 是supermum记录 */ ))
		    && (wait || !lock_get_wait(lock) /*请求wait锁，并且已有的lock上没有wait标记 */ )
		    && lock_mode_stronger_or_eq(lock_get_mode(lock), mode) /*原来的lock mode已经强于 请求的mode了 */ ) {
	...
	const trx_t*		trx)	/*!< in: our transaction */
{
	...
	for (lock = lock_rec_get_first(block, heap_no);
		...
		//在已有lock的基础上请求mode锁，锁是否冲突，如果冲突,需要等待
...
/*********************************************************************//**
确认是否有事物在辅助索引index上有implicit x-lock
					...
					trx mutex */
{
	...
	*/
	...
	if (type_mode & LOCK_WAIT) {
		...
		/* 标记lock->flag的 LOCK_WAIT标记 */
		lock_set_lock_and_trx_wait(lock, trx);
	}

	/* 事务中所有的锁,链表，如果当前事务是活跃事务，那么这个lock就有可能是implicit lock */
...
/*********************************************************************//**
给block上的第heap_no条记录添加一个type_mode锁,
注意锁重用
...
尝试给第heap_no条记录快速加锁,
条件
1，没有显式锁在page上，或者page上只有一个锁，并且被当前transaction拥有.
...
lock_rec_lock_fast(
/*===============*/
	ibool			impl,	/*!< in: impl 表示加隐式锁，其实也就是不加锁 if TRUE, no lock is set
	...
	que_thr_t*		thr)	/*!< in: query thread */
{
	...
	/* 从全局 Lock_sys->hash 中拿到第一个锁的，也就是 Hash 桶的第一个 node */
	...
	if (lock == NULL) {
		/* page上没有锁 */
		if (!impl) {
			/* 显式锁，Note that we don't own the trx mutex. */
			lock = lock_rec_create(
	...
	que_thr_t*		thr)	/*!< in: query thread */
{
	...
	if (lock_rec_has_expl(mode, block, heap_no, trx)) {
		...
		nothing */
			...
			block, heap_no, trx)) {
     /* 如果没有更强级别的锁，就要进行锁冲突判断，如果有锁冲突就需要入队列等待，并且还要进行死锁检测 */
		...
		/* 
		如果行数据上没有更强级别的锁，也没有冲突的锁，并且加的不是隐式锁
...
/*********************************************************************//**
对记录(行)加锁，如果不能马上获得锁，就进入等待队列
行记录加锁根据事务和page来进行。这里有两点需要注意：
1，对记录进行加锁，是否为implicit lock
2，锁对象lock_t是否可以重用(锁重用的前提是同一事务锁住同一个页面中的记录，并且锁模式相同，这个重用是指同一个页面的lock_t的bitmap中设置相应的bit而已)
...
/*==========*/
	ibool			impl,	/*!< in: if TRUE, no lock is set
					if no wait is necessary: we
					assume that the caller will
					set an implicit lock 是否为implicit lock，
					若是implicit lock，并且当前没有该锁的冲突模式存在，则不需要产生一个lock_t对象，
					否则，通过函数lock_rec_create创建一个lock_t对象，并加入等待队列，这样设计是为了当不存在锁冲突时，
					可以减少锁的创建*/
	ulint			mode,	/*!< in: lock mode: LOCK_X or
					LOCK_S possibly ORed to either
					LOCK_GAP or LOCK_REC_NOT_GAP */
	const buf_block_t*	block,	/*!< in: buffer block containing
					the record */
	ulint			heap_no,/*!< in: heap number of record，行的序号，也就是bitmap的序号 */
	dict_index_t*		index,	/*!< in: index of record */
	que_thr_t*		thr)	/*!< in: query thread */
{
	ut_ad(lock_mutex_own());

	/* 如果不是LOCK_S，那么必须已经获得表的LOCK_IS锁 */
	ut_ad((LOCK_MODE_MASK & mode) != LOCK_S
	      || lock_table_has(thr_get_trx(thr), index->table, LOCK_IS));

	/* 如果不是LOCK_X，那么必须已经获得表的LOCK_IX锁 */
	ut_ad((LOCK_MODE_MASK & mode) != LOCK_X
	      || lock_table_has(thr_get_trx(thr), index->table, LOCK_IX));

	/* mode 参数只能是 LOCK_S或者LOCK_X */
	ut_ad((LOCK_MODE_MASK & mode) == LOCK_S
	      || (LOCK_MODE_MASK & mode) == LOCK_X);
	/* LOCK_GAP,LOCK_REC_NOT_GAP,0 三个里面选一个 */
	ut_ad(mode - (LOCK_MODE_MASK & mode) == LOCK_GAP
	      || mode - (LOCK_MODE_MASK & mode) == LOCK_REC_NOT_GAP
	      || mode - (LOCK_MODE_MASK & mode) == 0);
	...
	*/
	...
	case LOCK_REC_SUCCESS_CREATED:
		return(DB_SUCCESS_LOCKED_REC);
	case LOCK_REC_FAIL:
		/* lock_rec_lock_fast失败之后，就用lock_slow逻辑 */
	...
	const lock_t*	wait_lock)	/*!< in: waiting record lock */
{
	...
	heap_no = lock_rec_find_set_bit(wait_lock);/* 首条被lock的记录 */
	...
	for (lock = lock_rec_get_first_on_page_addr(space, page_no);
		...
		if (heap_no < lock_rec_get_n_bits(lock)
		    && (p[bit_offset] & bit_mask) /* rec被lock了 */
		    && lock_has_to_wait(wait_lock, lock)) /* 在lock的基础上请求wait_lock是需要wait的 */ {
...
/*************************************************************//**
给WAITING状态的lock授权，并且取消它的waiting状态
...
/*************************************************************//**
将in_lock从in_lock->trx_lock->trx_locks中移除来
					...
					if they are now qualified to it */
{
	...
	/* trx_lock_t和lock_t不是一个类型的,
	trx_lock_t包含一个事务所有的锁信息*/
	...
	/* 从lock_sys->rec_hash中移除in_lock->hash构成的链表 */
	HASH_DELETE(lock_t, hash, lock_sys->rec_hash,
		    lock_rec_fold(space, page_no), in_lock);

	/* 从trx_lock->trx_locks中移除in_lock->trx_locks */
	...
	for (lock = lock_rec_get_first_on_page_addr(space, page_no);
		...
		if (lock_get_wait(lock) /* LOCK_WAIT标记 */
	...
	const buf_block_t*	block)	/*!< in: page to be discarded */
{
	...
	/* lock_sys->rec_hash中首个符合 space,page_no的lock */
	lock = lock_rec_get_first_on_page_addr(space, page_no);
...
/*************************************************************//**
从block上继承锁到heir_block上去，但是不能继承LOCK_INSERT_INTENTION
						...
						donating record */
{
	...
	for (lock = lock_rec_get_first(block, heap_no);
		...
		if (!lock_rec_get_insert_intention(lock) /* lock上没有LOCK_INSERT_INTENTION标记 */
		    && !((srv_locks_unsafe_for_binlog /*默认false*/
			  || lock->trx->isolation_level     /*默认 TRX_ISO_REPEATABLE_READ*/
 			  <= TRX_ISO_READ_COMMITTED )
			 && lock_get_mode(lock) ==
			 (lock->trx->duplicates ? LOCK_S : LOCK_X))) {

            /* 在heir_block上的第heir_heap_no条记录 上锁 */
			lock_rec_add_to_queue(
...
/*************************************************************//**
heir_heap_no记录从heap_no记录处继承GAP LOCK
						...
						on this record */
{
	...
	for (lock = lock_rec_get_first(block, heap_no);
		...
		if (!lock_rec_get_insert_intention(lock) /* 没有LOCK_INSERT_INTENTION标记 */
		    && (heap_no == PAGE_HEAP_NO_SUPREMUM
			    || !lock_rec_get_rec_not_gap(lock) /* 没有GAP标记 */ )) {
            
			/* 给block上的第heir_heap_no个记录创建相应的type_mode锁,标记有GAP和LOCK_REC */
			lock_rec_add_to_queue(
				LOCK_REC | LOCK_GAP | lock_get_mode(lock),
...
/*************************************************************//**
将donator上的第donator_heap_no记录上的lock移动到receiver上的第receiver_heap_no记录上去。
						...
						which gives the locks */
{
	...
	for (lock = lock_rec_get_first(donator, donator_heap_no);
		...
		/* 在receiver block上的第receiver_heap_no条记录上创建相应type_mode的lock */
		lock_rec_add_to_queue(
...
/*************************************************************//**
在整理page的时候，需要整理的对应的lock_rec的bitmap
在整理page的时候，会改变记录的heap_no
					...
					reorganized page */
{
	...
	for (lock = UT_LIST_GET_FIRST(old_locks); 
		 lock;
	     lock = UT_LIST_GET_NEXT(trx_locks, lock)) {
		...
		for (;;) {
			...
			if (lock_rec_get_nth_bit(lock, old_heap_no)) {
				...
				small for the new heap number! 
				  在block上设置相应的type_mode ,new_heap_no记录上的锁
				*/
				lock_rec_add_to_queue(
...
/*************************************************************//**
将block上的从rec开始的记录锁移动到new_block相应的(从first rec开始)记录上去
						...
						is the first record moved */
{
	...
	/* 根据block的space_id 和 page_no得到 lock_t对象*/
	for (lock = lock_rec_get_first_on_page(block); 
		 lock;
	     lock = lock_rec_get_next_on_page(lock)) {
		page_cur_t	cur1;
		page_cur_t	cur2;
		...
		if (page_cur_is_before_first(&cur1)) {
			/* 如果是infimum记录，就移动到下一条记录去 */
			page_cur_move_to_next(&cur1);
		}
		/* cur2设置到new_block的起始位置 */
		...
		while (!page_cur_is_after_last(&cur1)) {
			...
			if (lock_rec_get_nth_bit(lock, heap_no)) {
				...
				/* 
				type_mode为 lock的lock_mode
			    将lock的lock_mode添加到new_block的heap_no记录上去 */
				lock_rec_add_to_queue(
					type_mode, new_block, heap_no,
					lock->index, lock->trx, FALSE);
			}

			/* 各自的cursor都移动到下一条记录的位置 */
			page_cur_move_to_next(&cur1);
			page_cur_move_to_next(&cur2);
		}
	}
...
/*************************************************************//**
从block上的first record上开始移锁到new_block上的记录上去
Moves the explicit locks on user records to another page if a record
						...
						were copied */
{
	...
	for (lock = lock_rec_get_first_on_page(block); 
		...
		/* cur1指向infimum记录 */
		page_cur_set_before_first(block, &cur1);
		page_cur_move_to_next(&cur1); /* cur1指向page的第一条用户记录 */

		/* old_end 在new_block中的位置，记录在cur2中 */
		page_cur_position(old_end, new_block, &cur2);
		page_cur_move_to_next(&cur2); /* cur2移动到next */
...
/*************************************************************//**
B-TREE向右分裂时，将部分的锁信息从left node 的部分锁信息移动到right node 去
	...
	const buf_block_t*	left_block)	/*!< in: left page */
{
	...
	of the right page 
	   将原来页中的记录supremum持有的锁移动到新页的记录supremum
	*/
	...
	of the infimum on right page 
	   ...
	   将 right_block 上的 heap_no(首条用户记录)记录上的锁信息复制到left_block上的PAGE_HEAP_NO_SUPREMUM 记录上
	*/
						...
						NOT the root! */
{
	...
	/* 
	将block上的supremum上的锁复制到new_block上的supermum上去
...
/*===================*/
	...
	const rec_t*		orig_pred,	/*!< in: original predecessor
						...
						before merge,合并前left page上的supermum记录的前一条记录 */
						...
						which will be discarded */
{
	...
	/* left_next_rec 实际上right page的第一条记录 */
...
/*************************************************************//**
将rec删除之后，rec的next_rec继承rec的锁定范围
...
/*********************************************************************//**
在记录更新的时候，新记录比原记录大，不能进行原地更新，
需要先将old_rec上的锁信息保存到infimum record上去
...
transaction was chosen as a victim and rolled back or no deadlock found.
死锁检测：
算法是深度优先搜索，如果在搜索过程中发现有环，就说明发生了死锁，
为了避免死锁检测开销过大，
如果搜索深度超过了 200（LOCK_MAX_DEPTH_IN_DEADLOCK_CHECK)也同样认为发生了死锁。
@return id of transaction chosen as victim or 0 */
static
trx_id_t
lock_deadlock_check_and_resolve(
...
/*********************************************************************//**
创建和初始化表锁对象，产生的lock_t对象加入到dict_table_t的locks链表中去,
通过此链表可以遍历处table对象上所有的锁信息。
可以对表加任何类型的锁，S,X表示需要锁住表，即当前层，对表加IS,IX意向锁表示
需要对下层进行加锁。
	...
	trx_t*		trx)	/*!< in: trx */
{
	...
	/* 将lock->trx_locks添加到trx->lock.trx_locks中去 */
	UT_LIST_ADD_LAST(trx_locks, trx->lock.trx_locks, lock);

	/* 将lock->un_member.tab_lock.locks添加到table->locks中去  */
...
/*************************************************************//**
从trx->autoinc_locks或者trx->lock.trx_locks中移除 lock
...
/*=======================*/
	...
	que_thr_t*	thr)	/*!< 请求table上的锁的thr ，in: query thread */
{
	trx_t*		trx;
	lock_t*		lock;
	trx_id_t	victim_trx_id;
	...
	que_thr_t*	thr)	/*!< in: query thread */
{
	...
	to/from trx_t::table_locks. 
	  确认事务trx是否已经在table上已经持有相同或者更强级别的锁了。
	  如果已经持有，那就直接返回成功
	*/
...
/*********************************************************************//**
从trx->lock.trx_locks移除某个table的所有的lock
...
#ifdef UNIV_DEBUG
/*********************************************************************//**
从trx->lock.table_locks查找find_lock
...
/*********************************************************************//**
这个函数的作用是在insert之前做一些检查																	   
用来判断next_rec(参数中的rec的next_rec)上的锁，

参数inherit用来判断是否在调用函数lock_update_insert来对已经锁定的范围进行更新
...
/*===========================*/
	ulint		flags,	/*!< in: if BTR_NO_LOCKING_FLAG bit is
				set, does nothing */
	const rec_t*	rec,	/*!< 记录将要插入在rec后面，in: record after which to insert */
				...
				record */
{
	...
	trx = thr_get_trx(thr);
	/* next_rec是当前要插入记录的下一条记录，也是已有记录的下一条记录 */
...
/*********************************************************************//**
将implicit lock转成 explicit lock
	...
	const ulint*		offsets)/*!< in: rec_get_offsets(rec, index) */
{
	...
	if (trx_id != 0) {
		...
		explicit x-lock set on the record, set one for it 
		  事务依旧是活跃事务，并且没有显示锁加在rec上，那就给rec加上锁
		*/
		...
		if (impl_trx != NULL
			...
			lock_rec_add_to_queue(
				type_mode, block, heap_no /* bitmap 对应的位*/, index,
				impl_trx, FALSE);
		}
...
/*********************************************************************//**
在更新(删除)聚集索引之前判断记录上是否有其他事务持有锁，如果有其他事务持有锁，那就要等待
	...
	que_thr_t*		thr)	/*!< in: query thread */
{
	...
	If a transaction has no explicit x-lock set on the record, set one for it 
	 将事务在记录rec上的implicit lock 转化为 explicit lock
	*/
...
/*********************************************************************//**
在更新(删除)之前判断辅助索引记录上是否有其他事务持有锁，如果有其他事务持有锁，那就要等待
...
#ifdef UNIV_DEBUG
/*******************************************************************//**
trx_list中的任何一个trx上的锁是锁住 table或者table上的records的
	...
	const trx_list_t*	trx_list)	/*!< in: trx list to check */
{
	...
	for (trx = UT_LIST_GET_FIRST(*trx_list);
	     trx != NULL;
	     trx = UT_LIST_GET_NEXT(trx_list, trx) /*就是trx = trx->trx_list->next */ ) {
		...
		for (lock = UT_LIST_GET_FIRST(trx->lock.trx_locks); /* trx->lock.trx_locks.start */
		     lock != NULL;
		     lock = UT_LIST_GET_NEXT(trx_locks, lock)) {
...
/*******************************************************************//**
返回UT_LIST_GET_LEN(table->locks) > 0
...
/*******************************************************************//**
trx在第heap_no上是否有exclusive lock
