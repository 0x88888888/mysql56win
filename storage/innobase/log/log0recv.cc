...
从redo log group的文件中 获取最大的ckeckpoint lsn,
innodb 在redo log文件中保存两个checkpoint，
所以需要拿到这两个checkpoint最大的那一个
...
/*******************************************************************//**
根据日志类型不同调用不同的函数进行恢复 ，
recv_parse_or_apply_log_rec_body返回值到重做日志的结束位置，
如果重做日志不全，则返回NULL值，需要特别注意的是，函数参数page为NULL时，
表示仅分析(parse)重做日志
...
/*********************************************************************//**
根据(space , page_no)查找hash表中重做日志的recv_addr_t对象，
若不存在就创建recv_addr_t对象，并将其链接到recv_addr_t中的recv_t链表中
...
/*******************************************************************//**
添加解析好的redo log(recv对象)到recv_sys->addr_hash中
日志由recv_parse_or_apply_log_rec_body去解析
...
根据space_id,page_no,调用recv_parse_or_apply_log_rec_body
将hash表中的redo log 应用到page进行恢复。
在对页进行恢复时，首先判断也的LSN，当页的LSN大于待恢复重做日志的LSN时，
会跳过该日志。因为这表示页已经写入到磁盘或者之前已经恢复过该页。
...
#ifndef UNIV_HOTBACKUP
/*******************************************************************//**
读取相邻的32个page，若page存在于recv_sys的哈希表中，并且该页也不在缓存中，
那么记录该页的page_nos到数组page no中，在完成对32个页检查后，
调用buf_read_recv_pages，读取存放于数组page_nos中页的编号,然后进行异步读取，最后进行恢复
...
/*******************************************************************//**
回放redo log(recv_sys->addr_hash)中的日志
...
/*******************************************************************//**
解析一条redo日志,将分析后的日志加入到哈希表中，并不直接replay
恢复时，得到日志的type,space,page_no信息
