...
#define LOG_BUF_WRITE_MARGIN	(4 * OS_FILE_LOG_BLOCK_SIZE /* 512 */ )
...
/****************************************************************//**
遍历各个buf_pool_instances，找到最oldest_modification的那个
Returns the oldest modified block lsn in the pool, 
or log_sys->lsn if none exists.
	...
	ulint	len)
{
	...
	while (log_sys->n_pending_writes != 0
		...
		/*
		      1.有线程正在写redo log
          或者 2.log_sys->buf_free 和 log_sys->buf_next_to_write 不在同一个log block中
		*/
	...
	/* store the last log block in buffer ,最后一个log block*/
	...
	/* reallocate log buffer，转成page 数量 */
	srv_log_buffer_size = len / UNIV_PAGE_SIZE + 1;
	mem_free(log_sys->buf_ptr); /* 老的内存释放 */

	log_sys->buf_ptr = static_cast<byte*>(
		mem_zalloc(LOG_BUFFER_SIZE + OS_FILE_LOG_BLOCK_SIZE));//分配内存
	log_sys->buf = static_cast<byte*>(
		ut_align(log_sys->buf_ptr, OS_FILE_LOG_BLOCK_SIZE)); //对齐
...
/************************************************************//**
写入字符串到log_sys对象,也就是说这个动作完成之后，
redo log还是依旧在内存中，没有写到磁盘上去
持有 log_sys->mutex
...
#ifndef UNIV_HOTBACKUP
/********************************************************//**
调用log_group_write_buf()之后会调用次函数设置group->lsn_offset和group->lsn
...
/******************************************************//**
只从innobase_start_or_create_for_mysql中调用一次
Initializes the log. */
UNIV_INTERN
void
log_init(void)
/*==========*/
{
	...
	/* 
		...
		起步的lsn不用，跳过去，这部分空间，做redo log 系统内部的用途
	*/
	...
	log_sys->buf_free = LOG_BLOCK_HDR_SIZE;  /*  12  */
					...
					used */
{
	...
	group->file_size = file_size;//每个文件的字节数量
...
loop:
	...
	if ((next_offset % group->file_size) + len > group->file_size) {
		...
		/* 
		  ...
		  then the below expression is < len which is ulint, 
	...
	*/
	...
	if (log_do_write) {
		...
		*/
		fil_io(OS_FILE_WRITE | OS_FILE_LOG, true, group->space_id, 0,
		       (ulint) (next_offset / UNIV_PAGE_SIZE), /* page_no */
		       (ulint) (next_offset % UNIV_PAGE_SIZE), /* offset in page */
			   write_len, buf,
		       group);
...
# endif
	...
	if (log_sys->n_pending_writes > 0) {
		...
		if (!flush_to_disk
			...
			/* 刚好已近有线程请求刷盘了，别人的刷盘请求已近包含了我的刷盘请求lsn */
	...
	if (!flush_to_disk
		...
		/* 不刷盘，Nothing to write and no flush to disk requested */
	...
	while (group) {
		...
		/* 刷盘 */
		log_group_write_buf(
			group, log_sys->buf + area_start,//起始位置
			area_end - area_start,//长度
			ut_uint64_align_down(log_sys->written_to_all_lsn, OS_FILE_LOG_BLOCK_SIZE),
			start_offset - area_start);

		/* 设置group->lsn = lsn和group->lsn_offset */
...
/****************************************************************//**
刷redo log到磁盘，直到lsn到new_oldest为止
				...
				at least to this lsn */
{
	...
	if (recv_recovery_on) {
		...
		将recv_sys->addr_hash中的日志数据apply，做恢复
		*/
	...
	ulint		field)	/*!< in: LOG_CHECKPOINT_1 or LOG_CHECKPOINT_2 */
{
	...
	fil_io(OS_FILE_READ | OS_FILE_LOG, true, group->space_id, 0,
	       field / UNIV_PAGE_SIZE /* page number */, 
		   field % UNIV_PAGE_SIZE /* offset in page */,
...
/******************************************************//**
将checkpoint的值写入到 redo log file的头部位置处
				...
				made to log files */
{
	...
	确保到oldest_lsn为止的redo log已经刷下去了
	*/
...
loop:
	...
	if (advance) {
		...
		/* 刷redo log 到磁盘，直到checkpoint到new_oldest为止 */
...
/********************************************************************//**
对redo log缓冲、dirty page，checkpoint进行检测，确保有足够的空间供新的查询使用
...
loop:
	...
	*/
	...
	/*
	  执行到此就是正常关闭了，不是快事关闭，做一次checkpoint
	*/
	if (!srv_read_only_mode) {
