...
sync_array_t中的每一个sync_cell对应一个os级别的event
...
struct sync_cell_t {
	...
	ulint		request_type;	/*!< lock type requested on the
					object，请求的锁类型，是s-lock，还是x-lock */
...
/** Synchronization array 
    用这个类去定义全局的wait array (sync_wait_array)对象
*/
struct sync_array_t {
	ulint		n_reserved;	/*!<  已经被使用掉的cell的数量 
							      number of currently reserved
					              cells in the wait array */
	ulint		n_cells;	/*!< array的长度，
...
全局的 wait_array,各种等待mutex,rw_lock的信息都放入这个数组里面
在sync_array_init中初始化
*/
	...
	sync_cell_t*	cell)	/*!< in: cell to search */
{
	...
	if (cell->request_type == SYNC_MUTEX) {
		...
		os_rmb;
		if (mutex_get_lock_word(mutex) == 0) {
			// mutex->lock_word==0，说明一个线程已经获得了X-lock
			return(TRUE);
		}
...
/**********************************************************************//**
检测sync_wait_array是否存在无限等待的mutex，如果有，就唤醒
...
/**********************************************************************//**
检测sync_wait_array是否存在无限等待的mutex
If the wakeup algorithm does not work perfectly at semaphore relases,
this function will do the waking (see the comment in mutex_exit). This
function should be called about every 1 second in the server.
...
/**********************************************************************//**
初始化sync_wait_array
Create the primary system wait array(s), 
they are protected by an OS mutex */
UNIV_INTERN
void
sync_array_init(
/*============*/
