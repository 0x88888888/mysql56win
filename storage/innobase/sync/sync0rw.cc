...
-X_LOCK_DECR < lock_word < 0:  有线程已经获得s_lock,而且有线程在等待x_lock.
                                Read locked, with a waiting writer.
			       (-lock_word) is the number of readers
			       that hold the lock.


lock_word <= -X_LOCK_DECR:  递归了  Recursively write locked. lock_word has been
...
# endif
    ...
    /* 
    rw_lock_list->list->next = rw_lock_list->start 
    rw_lock_list->list->prev = NULL

    rw_lock_list->start->list.prev  = lock
    ...
...
#ifdef UNIV_DEBUG
/******************************************************************//**
验证lock_word的范围、magic_n、waiters
...
/******************************************************************//**
获取s-lock,如果有别的线程已经获得x-lock或则在等待获取x-lock
那么本线程将spin一段时间
...
lock_loop:
	...
	lock->lock_word<=0，说明有线程已经获得了x-lock了
	*/
	os_rmb;
...
/******************************************************************//**
x-lock线程已经减掉X_LOCK_DECR，等待 s-lock线程退出
	...
	ulint		line)	/*!< in: line where requested */
{
	...
	/*
	  ...
	  lock->lock_word ==0,
	  如果第一个线程首次获得x-lock
	  并且lock->lock_word < 0，说明有其他线程正在使用s-lock
	*/
	...
	if (rw_lock_lock_word_decr(lock, X_LOCK_DECR)) {
		...
		*/
		...
		if (!pass && lock->recursive
			...
			/* Relock */
			if (lock->lock_word == 0) {
				lock->lock_word -= X_LOCK_DECR;//第一次递归获取
			} else {
				--lock->lock_word; //第三次及以上次递归获取
			}
					...
					RW_LOCK_EX */
{
	...
	while (info != NULL) {
		/*
		当前thread拥有该类型的lock，并且也没打算由别的线程来unlock
		*/
