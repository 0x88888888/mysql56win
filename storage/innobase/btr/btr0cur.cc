...
btr_cur_search_to_nth_level(
/*========================*/
	dict_index_t*	index,	/*!< in: index,根据哪个索引进行查询 */
	ulint		level,	/*!< in: the tree level of search，查询到高度为level就结束，查询叶子节点中的记录时，level为0 */
	const dtuple_t*	tuple,	/*!< in: data tuple; NOTE: n_fields_cmp in
				tuple must be set so that it cannot get
				compared to the node ptr page number field! */
	ulint		mode,	/*!< in: PAGE_CUR_L, ...;
				Inserts should always be made using
				PAGE_CUR_LE to search the position!,mode的有效值为PAGE_CUR_LE,PAGE_CUR_L,PAGE_CUR_GE,PAGE_CUR_G */
	ulint		latch_mode, /*!< 对页和页内进行何种latch并发控制
				...
				to protect the record! ,latch_mode的有效值为BTR_SEARCH_LEAF, BTR_SEARCH_TREE, 
				                                           BTR_NO_LATCHES, BTR_MODIFY_TREE,
														   BTR_CONT_MODIFY_TREE,
														   BTR_SEARCH_PREV,BTR_SEARCH_MODIFY_PREV */
	btr_cur_t*	cursor, /*!< in/out: tree cursor; the cursor page is
				s- or x-latched, but see also above!查询得到的结果记录 */
	ulint		has_search_latch,/*!< in: info on the latch mode the
				caller currently has on btr_search_latch:
				RW_S_LATCH, or 0,是否已经对btr_search_latch加上任何latch保护
				                 RW_S_LATCH：已经对btr_search_latch加上latch保护 */
	...
	mtr_t*		mtr)	/*!< in: mtr */
{
...
#ifdef BTR_CUR_ADAPT
	btr_search_t*	info;  /*  启用hash adapter index*/
#endif
	...
	ut_ad(!dict_index_is_ibuf(index) || ibuf_inside(mtr)); /* 不能是insert buffer */
	ut_ad(dtuple_check_typed(tuple));
	ut_ad(!(index->type & DICT_FTS)); /* 不能是fulltext index */
...
# endif /* BTR_CUR_HASH_ADAPT */
	...
	非叶子节点中只能使用PAGE_CUR_L和PAGE_CUR_LE两种模式
	*/
	...
	mtr_t*		mtr)	/*!< in/out: mini-transaction */
{
	...
	if (!rec && !page_cur_get_page_zip(page_cursor)
	    && btr_page_reorganize(page_cursor, cursor->index, mtr) /* 整理page，然后重新尝试insert */ ) {
...
/*************************************************************//**
在插入数据时，检查锁信息，并生成对应的undo日志，若检测到下一条记录已经被其他事物持有锁，则返回DB_WAIT_LOCK
...
/*======================*/
	ulint		flags,	/*!< in: undo logging and locking flags: if not
				...
				specified，
				flags: BTR_NO_LOCKING_FLAG 若含有BTR_NO_LOCKING_FLAG标志位，则表示插入后不需要对查询的记录加上一个锁。例如对于插入缓存(insert buffer)，不会对其进行并发的读取操作，因此不需要对插入的记录进行保护。
				flags: BTR_KEEP_SYS_FLAG若不含有该标志位时，更新记录的隐藏列roll_ptr。例如当插入的是辅助索引或者非叶子节点时，记录不含有隐藏列，因此需要设置该标志位。
				flags: BTR_NO_UNDO_LOG_FLAG若含有此标志位，表示不需要记录undo log，列入回滚时不需要产生undo log*/
	btr_cur_t*	cursor,	/*!< in: cursor on page after which to insert;
				cursor stays valid ,cursor指向插入前通过函数btr_cur_search_to_nth_level定位到待插入记录的前一条记录，其查询mode为PAGE_CUR_LE, latch_mode为BTR_MODIFY_LEAF */
	ulint**		offsets,/*!< out: offsets on *rec */
	mem_heap_t**	heap,	/*!< in/out: pointer to memory heap, or NULL */
	dtuple_t*	entry,	/*!< in/out: entry to insert,待插入的记录 */
	rec_t**		rec,	/*!< out: pointer to inserted record if
				succeed，当插入成功，返回插入后的记录 */
	big_rec_t**	big_rec,/*!< out: big rec vector whose fields have to
				be stored externally by the caller, or
				NULL ，当插入成功，并且插入的记录转化为大记录，返回大记录格式 */
	ulint		n_ext,	/*!< in: number of externally stored columns */
	que_thr_t*	thr,	/*!< in: query thread or NULL */
	mtr_t*		mtr)	/*!< in/out: mini-transaction;
...
#ifdef BTR_CUR_HASH_ADAPT
	/*  是通过自适应哈希索引定位到的cursor， 更新自适应的哈希索引 */
...
当判定插入索引的操作可能引起分裂时，
会进行悲观插入(btr_cur_pessimistic_insert)，
在做实际的分裂操作之前，会先对文件进行扩展，
并尝试预留(tree_height / 16 + 3)个Extent，
大多数情况下都是3个Extent。
	...
	mtr_t*		mtr)	/*!< in/out: mini-transaction */
{
	...
	*big_rec = NULL;
	...
	if (!(flags & BTR_NO_UNDO_LOG_FLAG)) {
		...
		of lack of space 
		  进行悲观插入之前，预留空间，以保证索引树的结构发生变化，
		  有足够的磁盘空间可以保证这次插入操作完成，
		  插入操作完成后，会释放这部分预留空间。
		*/
	...
	roll_ptr_t*	roll_ptr)/*!< out: roll pointer */
{
	...
	if (!dict_index_is_clust(index)) {
		...
		/* 修改非聚集索引，就不添加undo log了
		   We do undo logging only 
		   when we update a clustered index record */
...
/*************************************************************//**
乐观更新，更新的空间要足够，主键列没有发生改变，即更新不涉及主键列
不对主键更新
Tries to update a record on a page in an index tree. It is assumed that mtr
holds an x-latch on the page(已经获取page的X锁). 
The operation does not succeed if there is too
...
/*======================*/
	ulint		flags,	/*!< in: 同乐观插入相同,undo logging and locking flags */
	btr_cur_t*	cursor,	/*!< in: 指向需要更新的记录，cursor on the record to update;
				cursor stays valid and positioned on the
				same record */
	ulint**		offsets,/*!< out: offsets on cursor->page_cur.rec */
	mem_heap_t**	heap,	/*!< in/out: pointer to NULL or memory heap */
	const upd_t*	update,	/*!< in:保存更新操作发生变化列的新值， update vector; this must also
				contain trx id and roll ptr fields */
	ulint		cmpl_info,/*!< in:辅助索引的更新信息， compiler info on secondary index
				updates */
	que_thr_t*	thr,	/*!< in: query thread */
	trx_id_t	trx_id,	/*!< in: transaction id */
	mtr_t*		mtr)	/*!< in/out: mini-transaction; if this
...
any_extern:
	...
	/* 将btr_cursor_t转成page_cursor_t对象 */
	page_cursor = btr_cur_get_page_cur(cursor);
	...
	*/
	...
	/* Restore the old explicit lock state on the record 
	   从infimum记录上恢复锁信息到block的rec记录上去
	*/
...
/*************************************************************//**
在更新的过程中，一个supremum被创建，并且这个supremum是被更新record的前一个record
那么这个supremum需要继承被更新的record的锁
...
估算一个range内的记录数量
估算方法：
1,统计range查询时两次定位之间的记录的数量，即n_rows.
2,统计下层共有多少页：n_rows = n_rows * 每个页的平均记录数.
3,当查询得到叶子节点后，预估RANGE的记录数量：若树的层数小于等于2层，则返回
预估值为n_rows，则返回2*n_rows.
