					...
					not UNIV_SQL_NULL */
{
	...
	case DATA_DECIMAL:
		/* decimal以字符串格式存储,Remove preceding spaces */
	...
	ulint		len2)	/*!< in: data field length or UNIV_SQL_NULL */
{
	...
	if (mtype >= DATA_FLOAT /* DATA_FLOAT, DATA_DOUBLE, DATA_DECIMAL, DATA_VARMYSQL, DATA_MYSQL*/
	    || (mtype == DATA_BLOB
...
/*************************************************************//**
逻辑记录与物理记录之间进行比较
这个函数在page_cur_serach_with_match中会使用
...
/*==========================*/
	...
	const rec_t*	rec,	/*!< in: physical record which differs from
				...
				dtuple */
	const ulint*	offsets,/*!< in: array returned by rec_get_offsets() */
	ulint		n_cmp,	/*!< 进行比较的列的数量 in: number of fields to compare */
