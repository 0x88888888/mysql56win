					...
					in: n=rec_offs_n_fields(offsets) */
{
	...
	if (dict_table_is_comp(index->table)) {
		...
		const byte*	lens;
		dict_field_t*	field;
		ulint		null_mask; /* 用来标记已经解析掉的null列的数量是否为8的背数 */
		...
		*/
		do {
			...
			if (!(dict_field_get_col(field)->prtype
				...
				/* nullable field => read the null flag */
				if (UNIV_UNLIKELY(!(byte) null_mask)) {
					/* 说明有很多的nullable列，重新设置null_mask，并且nulls字节减1 */
					nulls--;
					null_mask = 1;
				}
				...
				if (*nulls & null_mask) {
					...
					the length to zero and enable the
					SQL NULL flag in offsets[]. */
					len = offs | REC_OFFS_SQL_NULL; /* 记录下偏移*/
					goto resolved;
				}
				/*当前列为null*/
				null_mask <<= 1;
...
resolved:
			rec_offs_base(offsets)[i + 1] = len; /* 记录下第i列的长度 */

		} while (++i < rec_offs_n_fields(offsets));


		/* 就是 offsets[4] = 各个记录的长度的偏移开始出 OR 上 REC_OFFS_COMPACT标记  */
		*rec_offs_base(offsets)
			= (rec - (lens + 1)) | REC_OFFS_COMPACT;

      /* compact格式到此结束 */
	} else {
		/* Redundan格式， Old-style record: determine extra size and end offsets */
		offs = REC_N_OLD_EXTRA_BYTES; /* 6 */

		/* rec的REC_OLD_SHORT处的值,1个字节就可以表示每个field的长度了 */
		if (rec_get_1byte_offs_flag(rec)) {
			...
			*rec_offs_base(offsets) = offs;
			...
			/* 需要2个字节才能表示每个field的长度 */
			offs += 2 * rec_offs_n_fields(offsets);

			/* 就是offsets[REC_OFFS_HEADER_SIZE  ] = offs*/
			*rec_offs_base(offsets) = offs;
			/* Determine offsets to fields */
			do {
...
/******************************************************//**
确定每个filed在rec中的偏移
offsets[2]和offsets[3]两个字段在调试(UNIV_DEBUG)时用。
...
/*=================*/
	const rec_t*		rec,	/* rec可以为NULL !< in: physical record */
	const dict_index_t*	index,	/*!< in: record descriptor */
	...
	mem_heap_t**		heap)	/*!< in/out: memory heap */
{
	...
	*/
	...
	/* 设置offsets[1] = n ,n为fields数量 */
	rec_offs_set_n_fields(offsets, n);

	/* 在offsets中设置每一个field的偏移 */
					...
					temporary file record */
{
	...
	for (i = 0; i < n_fields; i++) {
		...
		field = dict_index_get_nth_field(index, i);
		len = dfield_get_len(&fields[i]); /* 每一个filed的长度 */
