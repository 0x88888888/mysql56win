...
/*******************************************************************//**
purge线程正在运行，insert ,delete,update操作稍作延迟
...
/**************************************************************//**
将mysql格式的非null列转成innodb格式的非null列,存储到dfiled中去
...
/**************************************************************//**
将一行mysql格式的数据转成innodb格式的数据,存储到row对应的field中去
...
/*********************************************************************//**
构建一个innodb层的tuple对象，设置tuple中filed的类型
					...
					handle */
{
	...
	/* 给node构建files_list，设置各个field的类型 */
					...
					handle */
{
	...
	row_get_prebuilt_insert_row(prebuilt);
	node = prebuilt->ins_node;
	/* mysql层的格式转成innodb层的tuple格式 */
	row_mysql_convert_row_to_innobase(node->row, prebuilt, mysql_rec);

	savept = trx_savept_take(trx);

	/* prebuilt->ins_graph->thrs的首个节点 */
	thr = que_fork_get_first_thr(prebuilt->ins_graph);
