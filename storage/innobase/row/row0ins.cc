...
/***********************************************************//**
根据表上的每个索引，为每个索引创建一个entry,insert的时候，在每个索引上都得插入
...
/*****************************************************************//**
添加DATA_ROW_ID，DATA_TRX_ID_LEN，DATA_ROLL_PTR_LEN几个系统列到node中去
	...
	dtuple_t*	row)	/*!< in: new row (or first row) for the node */
{
	...
	/* Create templates for index entries 
	   在node中创建tuple_list，赋值给node->entry_list
	*/
...
/*******************************************************************//**
对已经标记为delete的聚集索引记录，消除delete标记，然后在原有空间上插入entry
				...
				latching any further pages */
{
	...
	*big_rec = NULL;
	...
	roll_ptr 
	根据entry和rec 构建出一个update用的upd_t对象
	*/
...
/*=====================*/
	...
	const dtuple_t*	entry,	/*!< 正在被insert的 tuple in: entry that is being inserted */
	const rec_t*	rec,	/*!< 被比较的聚集索引记录 in: clustered index record */
	ulint*		offsets)/*!< in/out: rec_get_offsets(rec) */
{
	...
	DB_TRX_ID, DB_ROLL_PTR. 
	   比较entry和rec的主键+DB_TRX_ID+DB_ROLL_PTR  是否一样
	*/
...
/***************************************************************//**
insert 记录到聚集索引，忽略外键检查,重复的主键不能插入
Tries to insert an entry into a clustered index, ignoring foreign key
constraints. 
If a record with the same unique key is found, the other
...
/*==========================*/
	ulint		flags,	/*!< in: undo logging and locking flags */
	ulint		mode,	/*!< in: BTR_MODIFY_LEAF(乐观) or BTR_MODIFY_TREE(悲观),
...
/***********************************************************//**
从insert into  ...values语句中获得values
	...
	que_thr_t*	thr)	/*!< in: query thread */
{
	...
	if (node->state == INS_NODE_ALLOC_ROW_ID) {
		...
		if (node->ins_type == INS_SEARCHED) {
			/* insert into ... select...这种语句类型 */
			row_ins_get_row_from_select(node);

		} else if (node->ins_type == INS_VALUES) {
			/* insert into ... values (...)这种语句类型 */
