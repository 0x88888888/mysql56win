...
/********************************************************************//**
创建第id个trx_undo_t对象，一个trx_undo_t对象，对应一个undo文件
	...
	mtr_t*		mtr)	/*!< in: mtr */
{
	...
	/* 根据undo 的type不同，挂载到rseg对象的不同的链表中去 */
add_to_list:
	...
	trx_rseg_t*	rseg)	/*!< in: rollback segment memory object */
{
	...
	for (i = 0; i < TRX_RSEG_N_SLOTS; i++) {
		...
		if (page_no != FIL_NULL
			...
			/* 创建第i个trx_undo_t对象，一个trx_undo_t对象，对应一个undo文件 */
	...
	mtr_t*		mtr)	/*!< in: mtr */
{
	...
	if (undo->size == 1
       ...
       /* undo log只涉及一个页，并且这个undo 日志只占这个页的小于3/4 的空间 */
