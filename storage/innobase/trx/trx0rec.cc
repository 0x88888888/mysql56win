...
/**********************************************************************//**
在undo_page中插入一条 insert的 undo 日志信息
	...
	mtr_t*		mtr)		/*!< in: mtr */
{
	...
	*ptr++ = TRX_UNDO_INSERT_REC;// 类型
	ptr += mach_ull_write_much_compressed(ptr, trx->undo_no);/* undo序号 */
	ptr += mach_ull_write_much_compressed(ptr, index->table->id);/* table id */
...
/*******************************************************************//**
根据undo log构造记录以前的版本，这也就是MVCC的实现过程
