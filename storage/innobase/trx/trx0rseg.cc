...
/****************************************************************//**
创建FSP_FIRST_RSEG_PAGE_NO(6)的页面
	...
	mtr_t*	mtr)		/*!< in: mtr */
{
	...
	/* 
	  ...
	  TRX_RSEG + TRX_RSEG_FSEG_HEADER: page 中 segment header处,
	  TRX_RSEG + TRX_RSEG_FSEG_HEADER 处为相应的inode信息
	*/
	...
	/* Reset the undo log slots,  TRX_RSEG_N_SLOTS==1024 */
...
/***************************************************************************
创建rollback segment（trx_rseg_t）对象
	...
	mtr_t*		mtr)		/*!< in: mtr */
{
	...
	*((trx_rseg_t**) trx_sys->rseg_array + rseg->id) = rseg;
	...
	/* page_no对应的为 rollback segment page(跳过 FIL_HEADER) */
	rseg_header = trx_rsegf_get_new(space, zip_size, page_no, mtr);

	/* 根据rollback segment page中的数据设置rseg对象  */
	...
	mtr_t*		mtr)		/*!< in: mtr */
{
	...
	for (i = 0; i < TRX_SYS_N_RSEGS; i++) {
		...
		/* 读取TRX_SYS PAGE中TRX_SYS_RSEGS数组中的第i个page_no */
		...
		if (page_no != FIL_NULL) {
			...
			/* 获取数组TRX_SYS_RSEGS的第i个 TRX_SYS_RSEG_SPACE值 */
					...
					UNDO tablespaces */
{
	...
	/* 拿到FSP_TRX_SYS_PAGE_NO(5)对应的block frame */
	sys_header = trx_sysf_get(&mtr);

	/*从FSP_TRX_SYS_PAGE的TRX_SYS_RSEGS数组中获取  */
	for (i = 0; i < TRX_SYS_N_RSEGS; i++) {
		ulint	page_no;
		ulint	space;
