	...
	mtr_memo_slot_t*	slot)	/*!< in: memo slot */
{
	...
	if (slot->object != NULL && slot->type == MTR_MEMO_PAGE_X_FIX) {
		...
		/*将buf_block添加到buf_pool的flush list中去*/
		buf_flush_note_modification(block, mtr);
	}
}

/**********************************************************//**
将mtr中的dirty page添加到buf_pool的flush list中去
...
/************************************************************//**
将dirty page添加到 buf_pool的 flush list中去
	...
	mtr_t*	mtr)	/*!< in/out: mtr */
{
	...
	if (mtr->n_log_recs > 1) {
		/* 多条记录被修改，加上 MLOG_MULTI_REC_END标记*/
