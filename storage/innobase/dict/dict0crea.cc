...
/*****************************************************************//**
创建将要被插入到 SYS_TABLES表中的 一个dtuple_t对象，
					...
					tuple is allocated */
{
	...
	dfield_set_data(dfield, ptr, 8);//设置 DICT_COL__SYS_TABLES__ID列的数据
	...
	dfield_set_data(dfield, ptr, 4);// 设置DICT_COL__SYS_TABLES__N_COLS列的数据
	...
	dfield_set_data(dfield, ptr, 4);//设置DICT_COL__SYS_TABLES__TYPE列的数据
	...
	dfield_set_data(dfield, ptr, 4);//设置space_id的数据
	/*----------------------------------*/
...
/*****************************************************************//**
创建将要被插入到 SYS_COLUMNS表中的 一个dtuple_t对象，
Based on a table object, this function builds the entry to be inserted
...
/*==========================*/
	const dict_table_t*	table,	/*!< in: table */
	ulint			i,	/*!< in: column number  ,应该是table对象的第i列*/
	mem_heap_t*		heap)	/*!< in: memory heap from
					which the memory for the built
					tuple is allocated */
{
...
/****************************************************************//**
创建SYS_FOREIGN和SYS_FOREIGN_COLS 表
...
/****************************************************************//**
创建表SYS_TABLESPACES和SYS_DATAFILES
...
/*=====================================*/
{
	...
	/* 
	  We always want SYSTEM tables to be created inside the system tablespace. 
	  是否一个表一个ibd文件
	*/
	srv_file_per_table_backup = srv_file_per_table;
	srv_file_per_table = 0;
