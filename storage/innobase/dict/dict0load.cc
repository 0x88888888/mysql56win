...
/********************************************************************//**
从SYS_TABLE,SYS_COLUMN,SYS_INDEX,SYS_FIELD中加载<name>表
				...
				table and its indexes' definition */
{
	...
	mtr_start(&mtr);
	/*
	先找到系统表SYS_TABLES,根据这个表结构，拼一个记录出来，
	通过这条记录，去SYS_TABLE中查询想要的表<SYS_TABLES>
	*/
	sys_tables = dict_table_get_low("SYS_TABLES");
	sys_index = UT_LIST_GET_FIRST(sys_tables->indexes);
	ut_ad(!dict_table_is_comp(sys_tables));
	...
	*/
	...
	/*
	 设置搜索列信息，因为主键是NAME列，所以直接就通过这个列来搜索即可，
	 找到唯一想要查找的表信息，如果没有找到那就是没有<name>这个表了
	*/
	dfield_set_data(dfield, name, ut_strlen(name));
	dict_index_copy_types(tuple, sys_index, 1);

	/*
	  打开B+树，创建游标，对树进行查找，查找的模式是GE(大于等于)
	*/
	btr_pcur_open_on_user_rec(sys_index, tuple, PAGE_CUR_GE,
				  BTR_SEARCH_LEAF, &pcur, &mtr);
	/*
	  查找完成，取出游标找到的相应的记录，当然，如果没有找到的话，记录就是特殊值
	*/
	rec = btr_pcur_get_rec(&pcur);

	/*
	  处理没有找到，或者这条记录已经被删除(打了删除标记)的情况，
	  如果记录为特殊值，则关闭游标以及B+树，退出，返回NULL值
	*/
	if (!btr_pcur_is_on_user_rec(&pcur)
...
err_exit:
	...
	/*
	 解读找到的这条记录，创建一个table对象，这就是被加载的原始的表信息，
	 不包含列，索引，外键等信息
	*/
	...
	/*
	  对SYS_TABLE的搜索完成，关闭游标，执行mtr_commit
	*/
	btr_pcur_close(&pcur);
	mtr_commit(&mtr);
