...
struct fil_node_t {
	...
	ibool		being_extended;
				...
				being extended. */
	//每回写一次，就加1，与flush_counter合在一起，可以用于判断是否被修改
...
struct fil_space_t {
	...
	ibool		stop_ios;/*!< TRUE if we want to rename the
				...
				requests on the file，在重命名的时候需要暂停一下 */
	ibool		stop_new_ops;//删除tablespace的时候，设置为true,有些操作不被允许
				...
				* file flush
				...
				batches. */
	ulint		purpose;/*!< 本 table space的用途 FIL_TABLESPACE, FIL_LOG, or
				FIL_ARCH_LOG */
	UT_LIST_BASE_NODE_T(fil_node_t) chain;
				/*!< base node for the file chain */
	// size == page count of the tablespace
	...
	ulint		n_reserved_extents;
	            /* 为表空间操作预留的空闲extent的个数，比如B+树索引的节点分裂。
				   这些操作会导致表空间的size增加，为了防止增加后超过表空间size的最大值，
				   表空间预先增加这么多个extents */
				/*!< number of reserved free extents for
				ongoing operations like B-tree page split */
	//正在flush的情况，n_pending_flushes为正数
...
struct fil_system_t {
...
#endif /* !UNIV_HOTBACKUP */
	/*spaces存储 table space对象，用space id进行搜索 */
	hash_table_t*	spaces;		/*!< The hash table of spaces in the
					system; they are hashed on the space
					id */
	/*name_hash存储 table space对象，用space name进行搜索 */
	hash_table_t*	name_hash;	/*!< hash table based on the space
					name */
	UT_LIST_BASE_NODE_T(fil_node_t) LRU; // 数据fil_node,不包括 log_fils 和system tablespace 的fil_node
	...
	UT_LIST_BASE_NODE_T(fil_space_t) unflushed_spaces; /* 需要被刷到磁盘上去的表空间 */
	...
	if (node->open) {
		...
		if (fil_buffering_disabled(space)) {
			...
			ut_ad(fil_space_is_flushed(space));
			   ...
			   && fil_space_is_flushed(space) /* 比较node->modification_counter > node->flush_counter */
			   ) {
...
/*******************************************************************//**
创建fil_space_t对象，放入fil_system->space_list
...
/*******************************************************************//**
通过space的flag判断是否是压缩表，如果是就返回压缩表的page大小
...
/*******************************************************************//**
打开fil_system->space_list中包含的所有的文件
...
/*==========================================*/
{
	...
	/*
	  系统tablespace和redolog tablespace
	*/
	for (space = UT_LIST_GET_FIRST(fil_system->space_list);
		...
		for (node = UT_LIST_GET_FIRST(space->chain);
			...
			// printf("fil_open_log_and_system_tablespace_files_dbg: filename: %s  space_id: %d \n", node->name, node->space->id);
		}
	}
...
#ifdef __XQWDBG__
			printf("fil_close_all_files_dbg: file_name=%s  space_id=%x \n", node->name, node->space->id);
#endif //!__XQWDBG__ 
		}
					...
					must be >= FIL_IBD_FILE_INITIAL_SIZE */
{
	...
	/* 创建 fil_space_t对象，插入到fil_system->space_list中去 */
	success = fil_space_create(tablename, space_id, flags, FIL_TABLESPACE);

	/* 创建fil_node_t对象，插入到 space->chain中去 */
...
/*===================================*/
{
	...
	printf("fil_path_to_mysql_datadir:  %s\n", fil_path_to_mysql_datadir);
...
就是 space->n_reserved_extents -= n_reserved
*/
UNIV_INTERN
void
...
/********************************************************************//**
将page number 转成相应的file offset，然后进行文件操作
...
/*===*/
	...
	ulint	zip_size,	/*!< in: compressed page size in bytes;
				0 for uncompressed pages */
	ulint	block_offset,	/*!< in: offset in number of blocks, 就是page number */
	...
	void*	buf,		/*!< in/out: buffer where to store read data
				...
				appropriately aligned,  bpage->frame */
	void*	message)	/*!< in: message for aio handler if non-sync
				aio used, else ignored */
{
	ulint		mode;
	...
	is_log = type & OS_FILE_LOG;/* 是否是操作log 文件 */
...
# endif /* UNIV_LOG_DEBUG */
	...
	if (sync) {
		...
		mode = OS_AIO_LOG; /* 操作日志文件 */
	} else if (type == OS_FILE_READ
		   && !recv_no_ibuf_operations
		   && ibuf_page(space_id, zip_size, block_offset, NULL)) {

		mode = OS_AIO_IBUF; /* 操作change buffer 文件 */
	} else {
		mode = OS_AIO_NORMAL;/* 操作数据文件 */
	}
#else /* !UNIV_HOTBACKUP */
	ut_a(sync);
	mode = OS_AIO_SYNC;
	...
	*/
	...
	for (;;) {
		if (node == NULL) {
			...
			/* 
			  block_offset就是page number，
			  每过一个文件，需要减去这个文件含有的page number 总量,
			  因为上一个else if (node->size > block_offset) 比较的是单个文件内部的page number
			*/
	...
	/* 
	  ...
	  根据page_no和 page内偏移，计算出文件内的偏移
	*/

	if (!zip_size) {
		/*
		  block_offset == page number
		  UNIV_PAGE_SIZE_SHIFT == 14 , 14位,16k大小
		  根据page number 和页内偏移计算出在数据文件中的位置,
		  按照这种计算方法，这个page number 必须是node的file中的page number
		  如果是redo log file group呢？,必须在传入之前就处理过了
		*/
		offset = ((os_offset_t) block_offset << UNIV_PAGE_SIZE_SHIFT)
			+ byte_offset;
		// printf("fil_io_dbg: %s : page_no: %x  offset: %x \n", node->name, block_offset, offset);
	...
	mtr_t*			mtr)
{
	...
	/* 
	  ...
	  两个表都是file-per-table，就没必要rename了
	*/
