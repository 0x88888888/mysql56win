	...
	buf_flush_t	flush_type)/*!< in: type of flush */
{
	...
	/* 确保本线程已经获得额block->mutex */
	ut_ad(mutex_own(buf_page_get_mutex(bpage)));
	ut_ad(flush_type < BUF_FLUSH_N_TYPES);

	if (bpage->oldest_modification == 0
	    || buf_page_get_io_fix(bpage) != BUF_IO_NONE) {
		/* 判断bpage->io_fix！=BUF_IO_NONE，也就是任何线程不得持有页的latch  */
	    //没有被修改过的page
		return(false);
	}
...
/********************************************************************//**
将page从 buf_pool->flush_list中移去,但是不刷盘
	...
	bool		sync)		/*!< in: true if sync IO request */
{
    ...
    if (!is_uncompressed) {
		...
		if (flush_type != BUF_FLUSH_LIST) {
			flush = rw_lock_s_lock_gen_nowait(rw_lock, BUF_IO_WRITE);
		} else {
			/* Will S lock later */
			flush = TRUE;
		}
    ...
    if (flush) {
		...
		/* We are committed to flushing by the time we get here 
		   设置bpage->io_fix = BUF_IO_WRITE
		*/
		
		buf_page_set_io_fix(bpage, BUF_IO_WRITE);

		/* 设置bpage->flush_type=flush_type */
	...
	for (i = low; i < high; i++) {
		...
		if (flush_type != BUF_FLUSH_LRU
			...
			if (buf_flush_ready_for_flush(bpage, flush_type)
			    ...
			    && buf_flush_page(
					buf_pool, bpage, flush_type, false)/* 刷盘了*/ ) {
...
/******************************************************************//**
得到buf_pool->no_flush[type]这个event
...
/*******************************************************************//**
将buf_pool->flush_list的脏页刷到磁盘，一直刷到lsn_limit偏移为止
...

{
	...
	/* Flush to lsn_limit in all buffer pool instances 
	   逐个的buf_pool开始刷，直到刷到lsn_limit为止
	*/
	for (i = 0; i < srv_buf_pool_instances; i++) {
		buf_pool_t*	buf_pool;
		ulint		page_count = 0;
...
#ifdef UNIV_DEBUG
/******************************************************************//**
获得buf_pool->flush_list中 space_id = id 的数量
