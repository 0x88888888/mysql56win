...
得到 buf_pool的 LRU，free，flush_list 的长度信息
*/
UNIV_INTERN
void
...
/*===========*/
	...
	byte*		frame)		/*!< in: pointer to buffer frame  应该是尾部*/
{
	...
	ulint		mem_size)	/*!< in: requested size in bytes */
{
	...
	*/
	...
	/*
	如果是linux就用 shmget分配内存，
	如果是windows就用 VirtualAlloc分配
	*/
	...
	*/
	...
	//这个size实际上page count = 0x20c80000 / 0x4000 = 33568
	// chunk->size 的名字叫frame count比较好
...
给buf_pool 分配 buf_pool_size 的内存
...
/*===================*/
	buf_pool_t*	buf_pool,	/*!< in: buffer pool instance */
	ulint		buf_pool_size,	/*!< in: size in bytes, 20M */
	ulint		instance_no)	/*!< in: id of the instance,   0,1 */
{
	ulint		i;
	buf_chunk_t*	chunk;
	...
	mutex_create(flush_list_mutex_key, &buf_pool->flush_list_mutex,
		     SYNC_BUF_FLUSH_LIST);
	//BUF_FLUSH_LRU==0         BUF_FLUSH_N_TYPES==3
					...
					to free */
{
	...
	while (bpage != NULL) {
		...
		if (state != BUF_BLOCK_FILE_PAGE) {
			...
			buf_page_free_descriptor(bpage);//ut_free(bpage)
		}
	...
	buf_page_t*	dpage)	/*!< in/out: destination control block */
{
	...
	//从buf_pool->LRU中移除bpage->LRU
	UT_LIST_REMOVE(LRU, buf_pool->LRU, bpage);

	if (b) {
		/* 在链表buf_pool->LRU中，将dpage插入到b后 */
		UT_LIST_INSERT_AFTER(LRU, buf_pool->LRU, b, dpage);
	} else {
		/* 在bpage没有前驱节点的时候，dpage做链表buf_pool->LRU的首节点 */
...
/********************************************************************//**
bpage的地址在&buf_pool->watch[0] ~~~ &buf_pool->watch[BUF_POOL_WATCH_SIZE]
之间的，就是sentinel了
...
/********************************************************************//**
将bpage移动到buf_pool的LRU最开始(hot)处
...
/********************************************************************//**
根据space_id和page_no得到 buf_page_t(buf_block_t)
从buf_pool或者从磁盘上读取page
...
/*=============*/
	...
	ulint		offset,	/*!< in: page number */
	ulint		rw_latch,/*!< in:读取页后，锁持有的latch，
						         可能值有
						         RW_S_LATCH, RW_X_LATCH, RW_NO_LATCH */
	buf_block_t*	guess,	/*!< in: 猜测可能的页，guessed block or NULL */
	...
	mtr_t*		mtr)	/*!< in: mini-transaction */
{
	...
	/*
	  根据space_id, page number 得到buf_pool
	*/
	buf_pool_t*	buf_pool = buf_pool_get(space, offset);

	if(offset == IBUF_HEADER_PAGE_NO) {
		printf("buf_page_get_gen:debug IBUF_HEADER_PAGE_NO\n");
	}
	...
	//得到page_no相应的 rw_lock 的fold
	fold = buf_page_address_fold(space, offset);
	//从buf_pool得到page的 rw_lock_t，然后可以在hash表中查询是否有这个page对象
	hash_lock = buf_page_hash_lock_get(buf_pool, fold);
loop:
	block = guess;
...
got_block:
	...
	case BUF_BLOCK_ZIP_DIRTY:
		...
		/* 根据space，page_no计算出page的hash值，缓存起来 */
...
/********************************************************************//**
从free list中弄一个buf_block_t对象出来，准备读取磁盘上的数据
...
/********************************************************************//**
从buf_pool->LRU或则buf_pool->free_list中得到一块block
	...
	mtr_t*	mtr)	/*!< in: mini-transaction handle */
{
	...
	//从buf_pool->free或则buf_pool->LRU获取一个可用的block
	...
	if (block
		...
		//此space_id, page_no 已经存在于 hash_table中了
#ifdef UNIV_IBUF_COUNT_DEBUG
		ut_a(ibuf_count_get(space, offset) == 0);
#endif
#if defined UNIV_DEBUG_FILE_ACCESSES || defined UNIV_DEBUG
