...
/****************************************************************//**
page_no是否属于doublewrite buffer中	
...
/****************************************************************//**
获取doublewrite buffer的fseg信息所在的page，TRX_SYS_PAGE_NO(5)
...
/****************************************************************//**
从TRX_SYS_PAGE中读取doublewrite相关的信息
				...
				header on trx sys page */
{
	...
	/*
	  double buffer中 第一个block的第一个page的 page number ，之后连续的64个page都是属于这个block的
	*/
...
/****************************************************************//**
innodb启动的时候用，为doublewrite准备好 FSEG和相应的page
...
start_again:
	...
	if (mach_read_from_4(doublewrite + TRX_SYS_DOUBLEWRITE_MAGIC)
		...
		/* 
		  ...
		  在TRX_SYS_PAGE中，已经标记了DOUBLEWRITE的信息了,
		  只需要读进来，初始化就可以了
		*/
	...
	/* 下面开始创建 DOUBLEWRITE的信息，并且写入TRX_SYS_PAGE中 */
	...
	/*创建doublewrite的fseg，并且写入到TRX_SYS_PAGE */
	...
	/* 为doublewrite分配页面空间,在doublewrite只用2个extent的page，
	但是还是申请了2个extent的page加上32个fragment page，
	是因为doublewrite中不使用segment的fragment page，
	不使用fragment page，是不能保证fragment page是物理连续的*/
...
/****************************************************************//**
在数据库启动的时候，从TRX_SYS PAGE_NO(5)初始化double write buffer
	...
	bool		load_corrupt_pages)
{
   ...
   /* 从各自的page_no那里读取 1M的量到buf(也就是buf_dblwr->write_buf)中 */
