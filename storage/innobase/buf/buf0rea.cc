	...
	ulint	offset)	/*!< in: page number */
{
	...
	*err = DB_SUCCESS;
	...
	if (space == TRX_SYS_SPACE && buf_dblwr_page_inside(offset)) {
		//通常不会去读取 doublewrite buffer page
	...
	if (ibuf_bitmap_page(zip_size, offset)
	    || trx_sys_hdr_page(space, offset)) {
         /* insert buffer bitmap page之类的系统页，必须同步操作 */
	...
	completed 
	从buf_pool的free list中弄一个buf_block_t对象出来，准备读取磁盘上的数据
	*/
...
NOTE 1: the calling thread may own latches on
pages: to avoid deadlocks this function must be written such that it cannot
end up waiting for these latches! 

NOTE 2: the calling thread must want
				...
				routine */
{
	...
	if (ibuf_bitmap_page(zip_size, offset)
	    || trx_sys_hdr_page(space, offset)) {
        /* 属于insert buffer 和 trx sys的page，不进行预读 */
	...
	/* low与high之间是一个buf_read_ahead_random_area(最大值为64)的区间，
	这种算法，也不怎么随机啊。*/
	...
	ibool	inside_ibuf)	/*!< in: TRUE if we are inside ibuf routine */
{
	...
	if (ibuf_bitmap_page(zip_size, offset)
	    || trx_sys_hdr_page(space, offset)) {
        /* insert buffer bitmap page之类的，不进行预读 */
		/* If it is an ibuf bitmap page or trx sys hdr, we do
		no read-ahead, as that could break the ibuf page access
		order */
...
/********************************************************************//**
异步读取数组page_nos中的page no对应的page
					...
					in the array */
{
	...
	for (i = 0; i < n_stored; i++) {
		...
		while (buf_pool->n_pend_reads >= recv_n_pool_free_frames / 2) {
			/* 还有多个read被pending当中，所以先切换到别的io线程去执行 */
			os_aio_simulated_wake_handler_threads();
