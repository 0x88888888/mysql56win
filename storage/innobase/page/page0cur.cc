...
用cursor不同的查找模式在page内查找
定位逻辑记录tuple在页中的记录，并通过cursor返回查询结果，
iup_matched_fields，iup_matched_bytes
ilow_matched_fields，ilow_matched_bytes 为二叉查找算法记录比较时，
左右各已经匹配的字段数量和字节数量
由于page directory是稀疏的，因此首先通过二叉查找定位记录所在的slot，
之后再又通过二叉查找slot中的记录，最后定位到
*/
UNIV_INTERN
void
...
# endif
	...
	low = 0;
	up = page_dir_get_n_slots(page) - 1; // 槽数量

	/* Perform binary search until the lower and upper limit directory
	slots come to the distance 1 of each other 

	二分查找,while循环,直至找到up与low相邻近
	slot为中间槽
	mid_rec为中间槽中的中间一条记录

	cmp_dtuple_rec_with_match:将tuple中每一列与mid_red中每一列做比较,
	                          如果相等继续其他列
	*/
	...
	while (up - low > 1) {
		...
		/*
		  ...
		  cur_matched_bytes = min(low_matched_bytes, up_matched_bytes)
		*/
		ut_pair_min(&cur_matched_fields, &cur_matched_bytes,
			    low_matched_fields, low_matched_bytes,
			    up_matched_fields, up_matched_bytes);

		/* 从mid_rec计算出各个field的offsets */
...
up_slot_match:
	...
	while (page_rec_get_next_const(low_rec) != up_rec) {
		...
		/*
		 解析出mid_rec的field的长度，保存到offsets
		*/
		offsets = rec_get_offsets(mid_rec, index, offsets,
					  dtuple_get_n_fields_cmp(tuple),
					  &heap);
...
/*====================*/
	...
	ulint*		offsets,/*!< offsets为rec记录中所有fields在rec中的偏移 
						in/out: rec_get_offsets(rec, index) */
	mtr_t*		mtr)	/*!< in: mini-transaction handle, or NULL */
{
	...
	/* 1. Get the size of the physical record in the page 
	   整条记录的size，包括data_size和extra_size
	*/
	...
	/* free_rec=位置page + PAGE_FREE,
	 首先从PAGE_FREE处得到空闲位置，看是否有足够的空间
	 */
	...
	if (UNIV_LIKELY_NULL(free_rec)) {
		...
		/*
		  rec_offs_init就是rec_offs_set_n_alloc(offsets, (sizeof offsets) / sizeof *offsets)
		  设置foffsets_[0] = n_alloc;
		*/
		...
		if (rec_offs_size(foffsets) < rec_size) {
			/* PAGE_FREE处的第一个节点空间不够，那就直接从PAGE_HEAP_TOP处分配 */
			if (UNIV_LIKELY_NULL(heap)) {
				mem_heap_free(heap);
			}
		...
		/* PAGE_FREE处的第一个空闲节点有足够的空间可以用于本次insert */
...
#ifndef UNIV_HOTBACKUP
/**********************************************************//**
在mtr中写入一条record copy 的redo log.
	...
	mtr_t*		mtr)		/*!< in: mtr */
{
	...
	do {
		/* 获取rec的所有fields的偏移，将偏移记录在offsets数组中 */
		offsets = rec_get_offsets(rec, index, offsets,
					  ULINT_UNDEFINED, &heap);

		/* 复制rec到heap_top指向的buf中去 */
...
/***********************************************************//**
将cursor指向的记录删除，不是设置记录的delete flag=1，而是直接进入PAGE_GARBAGE
