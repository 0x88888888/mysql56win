...
/*************************************************************//**
设置PAGE_HEADER + PAGE_MAX_TRX_ID处的值为 trx_id
...
/************************************************************//**
从PAGE_HEAP_TOP处分配空闲的空间用于insert
...
/**********************************************************//**
创建一个索引页,初始化好infimum和supremum记录,
写好index page的page header
...
# error "PAGE_BTR_IBUF_FREE_LIST_NODE + FLST_NODE_SIZE > PAGE_DATA"
	...
	*/
	...
	tuple = dtuple_create(heap, 1);/* 给infimum分配一个dtuple实例 */
	dtuple_set_info_bits(tuple, REC_STATUS_INFIMUM);/*设置 tuple->info_bits 标记*/
	field = dtuple_get_nth_field(tuple, 0);

	dfield_set_data(field, "infimum", 8);/*数据*/
	...
	mtr_t*		mtr)	/*!< in/out: mini-transaction */
{
	...
	if (dict_index_is_sec_or_ibuf(index) && page_is_leaf(page)) {
		/* 辅助索引 or insert buffer tree并且page是叶子节点的pagen_recs */
	...
	mtr_t*		mtr)		/*!< in: mtr */
{
	...
	while (!page_cur_is_after_last(&cur1)) { /* cur1没到最后不算完 */

		rec_t*	cur1_rec = page_cur_get_rec(&cur1);
		rec_t*	ins_rec;
		/* cur1_rec记录中所有fields的偏移 */
...
#ifndef UNIV_HOTBACKUP
/*************************************************************//**
从block的rec开始copy，copy到new_block上去，直到block上的最后一条记录
...
/*************************************************************//**
B+树合并的时候用或者page_move_rec_list_start中调用

调用函数page_copy_rec_list_start将右页中的用户记录复制到左页，同时更新对应行锁的信息
就是将block中从记录rec开始到结尾的记录复制到new_block中去
...
split_rec.
将页(block)中的第一个记录移动到new_block中去
IMPORTANT: The caller will have to update IBUF_BITMAP_FREE
if new_block is a compressed leaf page in a secondary index.
This has to be done either within the same mini-transaction,
or by invoking ibuf_reset_free_bits() before mtr_commit().
...
/****************************************************************//**
分裂slot_no所管理的记录，以免一个slot负责太多的记录，降低排序速度
...
/*************************************************************//**
平衡slot_no和slot_no+1之间的记录数量,不要差距太多
	...
	ulint		slot_no)/*!< in: the directory slot */
{
	...
	if (up_n_owned > PAGE_DIR_SLOT_MIN_N_OWNED) {
		...
		old_rec = (rec_t*) page_dir_slot_get_rec(slot);/* 第slot个管理的第一个记录 */

		if (page_is_comp(page)) {
			new_rec = rec_get_next_ptr(old_rec, TRUE);
			/* 清空掉old_rec的 n_owned值 */
	...
	const rec_t*	rec)	/*!< in: the physical record */
{
	...
	if (page_is_comp(page)) {
		...
		for (i = 0; ; i++) {
			/* 获取第i个slot，虽然slots是逆序的，但是这个函数获取的却是正序的了 */
			slot = page_dir_get_nth_slot(page, i);
...
#ifndef UNIV_HOTBACKUP
/***************************************************************//**
确认slots的第一个slot是指向infimum记录的，
并且slots的最后一个slot是指向supremum的。
...
/*******************************************************//**
在leaf节点上删除一条记录，删除之后，将cursor移到下一条记录上去
