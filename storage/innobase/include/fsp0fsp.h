...
table space 的第一个page会使用到这个结构,其余的只填这个结构的一部分数据
每一个extent的第一个page也会使用到这个结构
*/
...
/* 指向xdes entry，本表空间内的 free extent */
#define	FSP_FREE            24	/* list of free extents */
/* 指向xdes entry，本表空间内的部分 free extent */
#define	FSP_FREE_FRAG		(24 + FLST_BASE_NODE_SIZE)
					/* list of partially free extents not
					belonging to any segment */
/* 指向xdes entry，本表空间内的extent，这些extent的所有page都被使用了 */
...
#define FSP_SEG_ID		(24 + 3 * FLST_BASE_NODE_SIZE)
					...
					segment id */
/* 指向inode page 管理本table space的 segments */
...
/*			FILE SEGMENT INODE
			==================
除去 page header(38)  + FLST_NODE_SIZE(12) 之后就是 inode entry 数组了
...
//                              FSEG_PAGE_DATA == FIL_PAGE_DATA == 38
#define FSEG_INODE_PAGE_NODE	FSEG_PAGE_DATA
					/* the list node for linking segment inode pages */
//                                            FLST_NODE_SIZE =  2 * FIL_ADDR_SIZE == 12
#define FSEG_ARR_OFFSET		(FSEG_PAGE_DATA + FLST_NODE_SIZE)
/*-------------------------------------*/
/*
 Inode Entry结构,一个Inode Entry 管理着一个segment
 inode type page 除去 FIL_HEADER(38)字节 + FSEG_INODE_PAGE_NODE 头之后的结构
*/
...
/* 指向xdes entry 链表,管理本segment的 extents */
#define	FSEG_FREE		12
...
/* 
FSEG_FRAG_ARR，数组，存放属于该segment的头32个page number

总是先从全局分配独立的Page，当填满32个数组项时，
就在每次分配时都分配一个完整的Extent，
并在XDES PAGE中将其Segment ID设置为当前值
*/
...
/*-------------------------------------*/
/*
  FSEG_INODE_SIZE为 INODE PAGE 中Inode Entry的大小
  16+ 3*16 + 32*4=16 + 48 + 128 = 192字节
*/
...
XDES结构会存在于FIL_PAGE_TYPE_FSP_HDR和FIL_PAGE_INODE类型的页面中
...
/** 
   ...
   XDES_SIZE = (12 + 12) + UT_BITS_IN_BYTES( 64 * 2 ) ＝ 24 + 128/8= 40 字节
*/
#define	XDES_SIZE							\
	(XDES_BITMAP							\
