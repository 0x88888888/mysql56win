...
/*
buf_chunk_t由 buf_block_t组成，buf_block_t 管理 buf_page_t,buf_page_t对应物理frame
*/
struct buf_page_t{
	/** @name General fields
	...
	*/
	...
	unsigned	state:BUF_PAGE_STATE_BITS; // BUF_PAGE_STATE_BITS = 3
...
# endif
	...
	ibool		in_flush_list;	/*!< 
					在buf_pool->flush_list时为true。			
					TRUE if in buf_pool->flush_list;
	...
	unsigned	freed_page_clock:31;/*!< the value of
					...
					mutex or latch */
	/* @} */
	unsigned	access_time;	/*!< 首次访问页面的时间，预读进来的时候是0
...
struct buf_block_t{
	...
	buf_page_t	page;		/*!< page information; 
							...
							so that buf_pool->page_hash can point
...
struct buf_pool_t{
...
#endif
	ulint		n_chunks;	/*!< number of buffer pool chunks,chunks数组长度  */
	buf_chunk_t*	chunks;		/*!< buffer pool chunks */
	ulint		curr_size;	/*!< current pool size in pages */
	//buffer pool 中所有pages的头结构快速定位的hash table
	...
	UT_LIST_BASE_NODE_T(buf_page_t) flush_list;
					...
					list,flush_list 就是 dirty page list */
	...
	ulint		freed_page_clock;/*!< a sequence number used
					...
					*/
	ibool		try_LRU_scan;	/*!< 
					当从LRU列表查找一个free block失败时，设置为false			
					Set to FALSE when an LRU
	...
	buf_page_t*	LRU_old;	/*!<指向LRU的old处(3/8) pointer to the about
