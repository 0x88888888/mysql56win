...
/** Block of log record data 
    虽然只有一个next成员，但是这个会动态分配对象大小
*/
struct recv_data_t{
	recv_data_t*	next;	/*!< 
							一个page的redo log 可能会大于16k
...
/** Stored log record struct 
   由于一个page在恢复时可能有多个redo log信息，
   因此需要通过链表对各个recv_t对象进行串联。
   每个recv_t中记录了该redo log的类型，长度，开始LSN，结束LSN，
   以及redo log的body，而body有recv_data_t数据结构定义，
   由于每个重做日志的body的最大长度为16k，所以对于大于16k的重做日志，同样需要连接
*/
struct recv_t{
...
/** Recovery system data structure 
    有一个recv_sys对象，负责管理用redo log的恢复工作
*/
struct recv_sys_t{
#ifndef UNIV_HOTBACKUP
	...
	byte*		last_block_buf_start;
				...
				preceding buffer */
	byte*		buf;	/*!< 从redo log中读取出来的数据，暂时存放到这里，
	...
	hash_table_t*	addr_hash;/*!< 
							   这个hash表存放着恢复的日志信息，每个成员为recv_addr_t对象,
							   用<space, page_no>去计算哈希值
