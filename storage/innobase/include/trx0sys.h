...
ibdata的第6个page，记录了InnoDB重要的事务系统信息
*/
...
/* 
用于存储128个回滚段位置，包括space id及page no。
每个回滚段包含一个文件segment（trx_rseg_header_create）
*/
...
#define	TRX_SYS_MYSQL_LOG_MAGIC_N_FLD	0	/*!< magic number which is
						...
						MySQL binlog info */
/*
事务提交时会将其binlog位点更新到该page中，
这里记录了在binlog文件中偏移量的高位的4字节
*/
...
*/
/*-------------------------------------------------------------*/
// 包含double write buffer的fseg header ,10 字节
...
#define TRX_SYS_DOUBLEWRITE_MAGIC	FSEG_HEADER_SIZE
						...
						buffer */
/*
double write buffer的第一个block(占用一个Extent)在ibdata中的开始位置，
连续64个page
*/
...
/*
用于兼容老版本，当该字段的值不为TRX_SYS_DOUBLEWRITE_SPACE_ID_STORED_N时，
需要重置dblwr中的数据
*/
...
struct trx_sys_t{
	...
	trx_id_t	max_trx_id;	/*!< The smallest number not yet
					...
					transaction number，下一个事务要使用的事务id */
#ifdef UNIV_DEBUG
	...
	trx_list_t	rw_trx_list;	/*!< List of active and committed in
					...
					当前活跃的事物(包括正处于内存提交的事物)链表，
					根据事物id逆序存放，即事物id大的存放在链表头部*/
	...
	trx_rseg_t*	const rseg_array[TRX_SYS_N_RSEGS];
					/*!< 回滚对象,所有回滚段都记录在这个数组里了，
					数组的值在函数trx_rseg_mem_create()中设置
