...
/* The rollback segment memory object 
  回滚段对象，
  事务开启时，会专门给他指定一个回滚段，
  以后该事务用到的undo log页，就从该回滚段上分配;

  事务提交后，需要purge的回滚段会被放到purge队列上(purge_sys->purge_queue)
*/
struct trx_rseg_t{
	/*--------------------------------------------------------*/
	ulint		id;	/*!< 回滚段id，rollback segment id == the index of
				its slot in the trx system file copy */
	ib_mutex_t		mutex;	/*!< mutex protecting the fields in this
				struct except id, which is constant */
	ulint		space;	/*!< 回滚段segment header 所在页的space id，
						  space where the rollback segment is
				           header is placed */
	ulint		zip_size;/* compressed page size of space
				in bytes, or 0 for uncompressed spaces */
	ulint		page_no;/*回滚段segment header 所在页的page number,
						  page number of the rollback segment
				          header */
	ulint		max_size;/* 最大 undo页的数量， maximum allowed size in pages */
	ulint		curr_size;/* 当前 undo页的数量， current size in pages */
	/*--------------------------------------------------------*/
	/* Fields for update undo logs,update undo 日志列表 */
	UT_LIST_BASE_NODE_T(trx_undo_t) update_undo_list;
					/* List of update undo logs */
	UT_LIST_BASE_NODE_T(trx_undo_t) update_undo_cached;
					/* 被cache的update undo 日志列表， List of update undo log segments
					cached for fast reuse */
	/*--------------------------------------------------------*/
	/* Fields for insert undo logs */
	UT_LIST_BASE_NODE_T(trx_undo_t) insert_undo_list;
					/* insert undo 日志列表,List of insert undo logs */
	UT_LIST_BASE_NODE_T(trx_undo_t) insert_undo_cached;
					/* 被cache的insert undo 日志列表, List of insert undo log segments
					cached for fast reuse */
	/*--------------------------------------------------------*/
	ulint		last_page_no;	/*!< 最近一次未被purge的undo日志header
								所在页的page no，
								Page number of the last not yet
					            purged log header in the history list;
					            FIL_NULL if all list purged */
	ulint		last_offset;	/*!< 最近一次未被purge的undo日志header
								所在页的业内偏移量。
								Byte offset of the last not yet
					            purged log header */
	trx_id_t	last_trx_no;	/*!< 最新未被purge的事务编号，
								Transaction number of the last not
					yet purged log */
...
管理undo log 的segment
undo log 的page header(在38字节的file header之后)
*/
/*-------------------------------------------------------------*/
...
#define	TRX_RSEG_FSEG_HEADER	(8 + FLST_BASE_NODE_SIZE)
					...
					this page is placed */
/*
undo slot数组，共1024个slot，值为FIL_NULL表示未被占用，
否则记录占用该slot的第一个undo page
*/
