...
/** 
大记录并不存放在dtuple_t结构中，而是存放在big_rec_t中
...
struct dtuple_t {
	ulint		info_bits;	/*!< info bits of an index record:
					...
					a data tuple，就是物理记录中的record header中的前4个bits */
	ulint		n_fields;	/*!< 记录中列的数量 number of fields in dtuple */
	ulint		n_fields_cmp;	/*!< 在记录进行比较时，仅比较这些数量的列，在记录比较函数(rem0cmp.*)中使用，
								     如在索引中进行搜索，仅判断这些数量的列。
								     其初始化在函数dtuple_create中完成，默认与n_field相同
...
struct big_rec_field_t {
	ulint		field_no;	/*!< field number in record,指针链接到下一部分数据 */
