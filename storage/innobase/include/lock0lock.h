...
#define LOCK_WAIT	256	/*!< 表示正在等待锁， Waiting lock flag; when set, it
				means that the lock has not yet been
				granted, it is just waiting for its
				turn in the wait queue */
/* Precise modes */
#define LOCK_ORDINARY	0	/*!< 表示 next-key lock ，锁住记录本身和记录之前的 gap， this flag denotes an ordinary
				next-key lock in contrast to LOCK_GAP
				or LOCK_REC_NOT_GAP */
#define LOCK_GAP	512	/*!< 表示锁住记录之前 gap（不锁记录本身）， when this bit is set, it means that the
				...
				of records */
#define LOCK_REC_NOT_GAP 1024	/*!< 表示锁住记录本身，不锁记录前面的 gap, this bit means that the lock is only on
...
#define LOCK_INSERT_INTENTION 2048 /*!<  插入意向锁, this bit is set when we place a waiting
