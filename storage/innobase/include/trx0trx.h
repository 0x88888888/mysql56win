...
struct trx_lock_t {
	...
	lock_t*		wait_lock;	/*!< 如果本事务正在等待某个lock，这个指针指向那个lock对象了
	...
	ibool		was_chosen_as_deadlock_victim;//在出现锁等待循环时，标记对应的是否是否被victim
	...
	ibool		cancel;		/*!< 
	                对应的事务是否被rollback或则deadlock了(被选为victim了)
...
struct trx_t{
	...
	* NOT_STARTED -> PREPARED -> (freed)
	...
	trx_state_t	state;/* 事物状态 */

	trx_lock_t	lock;		/*!< 本事务所持有和等待的所有的锁信息
	Information about the transaction locks and state. Protected by
	...
	*/
	const char*	op_info;	/*!< English text describing the
					...
					string */
	ulint		isolation_level;/*!< 隔离级别 TRX_ISO_REPEATABLE_READ, ... */
	ulint		check_foreigns;	/*!< 外键检查，normally TRUE, but if the user
	...
	ulint		has_search_latch;/* 事物是否持有自适应的全局的s-latch */
					/*!< TRUE if this trx has latched the
					search system latch in S-mode */
	ulint		search_latch_timeout;
					/*!< 若发现有事物持有的自适应哈希索引全局s-latch,
					则重置该函数，并在该值范围内事物不跨行持有适用自适应
					哈希索引，从而减少自适应哈希索引全局latch的竞争。
					...
					latch */
	trx_dict_op_t	dict_operation;	/**< 事物的DDL操作 @see enum trx_dict_op */
	...
	trx_id_t	no;		/*!< 
						当事物进入TRX_COMMITTED_IN_MEMORY时的最大事物id
	...
	time_t		start_time;	/*!< 事务切换到TRX_STATE_ACTIVE状态的时间， 
					time the trx state last time became
					TRX_STATE_ACTIVE */
	trx_id_t	id;		/*!< 事物id, transaction id */
	XID		xid;		/*!< X/Open XA transaction
					identification to identify a
					transaction branch */
	lsn_t		commit_lsn;	/*!< lsn at the time of the commit */
	table_id_t	table_id;	/*!<进行DDL操作表的id， Table to drop iff dict_operation
					== TRX_DICT_OP_TABLE, or 0. */
	/*------------------------------*/
	THD*		mysql_thd;	/*!< MySQL thread handle corresponding
					to this trx, or NULL */
	const char*	mysql_log_file_name;
					/*!< bin log 文件名称，if MySQL binlog is used, this field
	...
	ib_int64_t	mysql_log_offset;
					...
					binlog entry ,写入到二进制日志写入到的偏移*/
	/*------------------------------*/
	ulint		n_mysql_tables_in_use; /*!<当前SQL语句使用表的数量， 
	...
	UT_LIST_NODE_T(trx_t)
			trx_list;	/*!< 事务链表，list of transactions;
...
#endif /* UNIV_DEBUG */
	UT_LIST_NODE_T(trx_t)
			mysql_trx_list;	/*!<内部事务链表 list of transactions created for
	...
	dberr_t		error_state;	/*!< 0 if no error, otherwise error
					...
					by any mutex */
	const dict_index_t*error_info;	/*!< 当出现duplicate key error时,这个字段记录相应的索引
	...
	read_view_t*	read_view;	/*!< 事务持有的read_view对象链表，
	...
	UT_LIST_BASE_NODE_T(trx_named_savept_t)
			trx_savepoints;	/*!< savepoints set with SAVEPOINT ...,
					oldest first */
	/*------------------------------*/
	ib_mutex_t	undo_mutex;	/*!< 用于保护undo_mutex操作的互斥量，
	...
	trx_savept_t	last_sql_stat_start;
					/*!< 保存点，可用于回滚，由互斥量kernel_mutex保护，
					undo_no when the last sql statement
					was started: in case of an error, trx
					is rolled back down to this undo
					number; see note at undo_mutex! */
	trx_rseg_t*	rseg;		/*!<分配给事务的回滚段， rollback segment assigned to the
					transaction, or NULL if not assigned
					yet */
	trx_undo_t*	insert_undo;	/*!<insert undo日志 pointer to the insert undo log, or
					NULL if no inserts performed yet */
	trx_undo_t*	update_undo;	/*!<update undo日志 pointer to the update undo log, or
					NULL if no update performed yet */
	undo_no_t	roll_limit;	/*!< 回滚到的undo日志 least undo number to undo during
					a rollback */
	ulint		pages_undone;	/*!<回滚使用的undo页的数量 number of undo log pages undone
					since the last undo log truncation */
	trx_undo_arr_t*	undo_no_arr;	/*!<回滚时创建的槽，保存回滚的undo日志的undo no,
	...
	ulint		n_autoinc_rows;	/*!< no. of AUTO-INC rows required for
					...
					multi-row INSERTs */
	ib_vector_t*    autoinc_locks;  /* 自增用， AUTOINC locks held by this
