...
struct mtr_memo_slot_t{
	...
	void*	object;	/*!< 可以是rw_lock_t对象，也可以是buf_block_t对象
...
struct mtr_t{
	...
	/* 
	   在一个mtr中可能会修改多个page
	   用于存储该mtr持有的锁类型，每个page都有个latch持有在这里面，
       而且是根据修改页的次序存储的，避免造成死锁。
	*/
	dyn_array_t	memo;	/*!<
						保存mtr_memo_slot_t对象，
						slot对象中的type是latch信息
						slot对象中的object可以是rw_lock_t对象，也可以是buf_block_t对象
	...
	*/
	...
	MTR_LOG_SHORT_INSERTS：插入记录操作REDO，在将记录从一个page拷贝到另外一个新建的page时用到，此时忽略写索引信息到redo log中。（参阅函数page_cur_insert_rec_write_log）
	*/
	ulint		log_mode; /* specifies which operations should be
				logged; default value MTR_LOG_ALL */
	lsn_t		start_lsn;/* mini_transaction开始时的lsn
						  start lsn of the possible log entry for
				this mtr */
	lsn_t		end_lsn;/* mini_transaction结束时的lsn
