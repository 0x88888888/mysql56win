...
默认(srv_data_file_is_raw_partition)[i]=0



数据文件路径中有new或者newraw时，才可能会有 SRV_NEW_RAW或者SRV_OLD_RAW这两个值。
有bootstrap启动选项时，只会是srv_data_file_is_raw_partition[i] = SRV_NEW_RAW
(srv_data_file_is_raw_partition)[i] = opt_bootstrap ? SRV_NEW_RAW(1) : SRV_OLD_RAW(2);
*/
...
/**********************************************************************//**
Enqueues a task to server task queue and releases a worker thread, 
if there is a suspended one. */
UNIV_INTERN
void
