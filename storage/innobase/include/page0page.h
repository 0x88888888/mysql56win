...
/*-----------------------------*/
/* 
page type = FIL_PAGE_INDEX 的索引页信息
*/
// Page directory中的slot个数
#define PAGE_N_DIR_SLOTS 0	/* number of slots in page directory */
// 指向当前Page内已使用的空间的末尾偏移位置，即free space的开始位置
#define	PAGE_HEAP_TOP	 2	/* pointer to record heap top */
/*
Page内所有记录个数，包含用户记录，系统记录以及标记删除的记录，
同时当第一个bit设置为1时，表示这个page内是以Compact格式存储的
*/
...
#define	PAGE_LAST_INSERT 10	/* pointer to the last inserted record, or
				...
				for example */
/*
用于指示当前记录的插入顺序以及是否正在进行顺序插入，
每次插入时，PAGE_LAST_INSERT会和当前记录进行比较，
以确认插入方向，据此进行插入优化
*/
...
//PAGE_BTR_IBUF_FREE_LIST 偏移处记录 change buffer 中free状态的page
...
/*
在 PAGE_BTR_SEG_LEAF 和PAGE_BTR_SEG_TOP中10个字节的fseg信息包括
FSEG_HDR_SPACE	   4字节    描述该segment的inode page所在的space id （目前的实现来看，感觉有点多余…）
FSEG_HDR_PAGE_NO   4字节    描述该segment的inode page的page no
FSEG_HDR_OFFSET	   2字节    inode page内的页内偏移量
...
page上最小的用户记录前节点总是指向infimum，
page上最大的记录后节点总是指向supremum记录

注意:Heap number仅代表物理存储顺序，不代表键值顺序
*/
/* Heap numbers */
#define PAGE_HEAP_NO_INFIMUM	0	/* page infimum */
#define PAGE_HEAP_NO_SUPREMUM	1	/* page supremum */
// heap number 从2开始就是用户记录了
...
split_rec.
page_move_rec_list_start完成记录的移动 
IMPORTANT: The caller will have to update IBUF_BITMAP_FREE
if new_block is a compressed leaf page in a secondary index.
This has to be done either within the same mini-transaction,
or by invoking ibuf_reset_free_bits() before mtr_commit().
