...
/*
  每个page都有的头部(称为fil header),38个字节
*/
...
/*
只用于系统表空间的第一个Page，
记录在正常shutdown时安全checkpoint到的点，
对于用户表空间，这个字段通常是空闲的，
但在5.7里，FIL_PAGE_COMPRESSED类型的数据页则另有用途
*/
...
/*
FIL_PAGE_SPACE_ID 存储page所在的space id
*/
...
Inode Entry 描述占用该inode的segment的信息以及Extent链表管理


InnoDB通过Inode Entry来管理每个Segment占用的数据页
每个segment可以看做一个文件页维护单元。
Inode Entry所在的inode page有可能存放满，
因此又通过头Page维护了Inode Page链表。


FIL_PAGE_INODE类型的page用于管理数据文件中的segement，
每个索引占用2个segment，分别用于管理叶子节点和非叶子节点。
每个inode页可以存储FSP_SEG_INODES_PER_PAGE（默认为85）个记录
*/
#define FIL_PAGE_INODE		3	/*!< Index node */

//change buffer 中 free状态的 page
...
BITMAP page entry 描述一个page

用于跟踪随后的每个page的change buffer信息，
使用4个bit来描述每个page的change buffer信息。
由于bitmap page的空间有限，同样每隔256个Extent Page之后，
也会在XDES PAGE之后创建一个ibuf bitmap page。
*/
#define FIL_PAGE_IBUF_BITMAP	5	/*!< Insert buffer bitmap */

/*
FSP_TRX_SYS_PAGE_NO/FSP_FIRST_RSEG_PAGE_NO ibdata的第6个page，
记录了InnoDB重要的事务系统信息

InnoDB最多可以创建128个回滚段，
每个回滚段需要单独的Page来维护其拥有的undo slot，Page类型为FIL_PAGE_TYPE_SYS
...
/* 
FIL_PAGE_TYPE_FSP_HDR这种类型 在创建一个新的表空间时进行初始化(fsp_header_init)
...
/*
   XDES Entry ，描述一个extent
   XDES用于管理extent中所有page,
*/
...
#ifdef UNIV_LOG_ARCHIVE
/****************************************************************//**
Drops files from the start of a file space, 
so that its size is cut by the amount given. */
UNIV_INTERN
void
