...
/* The persistent B-tree cursor structure. 
   This is used mainly for SQL selects, updates, and deletes. 
   上层函数通常不直接调用btr0cur中的函数,而是通过btr_pcur_t对象来调用 btr0cur中的函数
   persistent cursor
*/
...
struct btr_pcur_t{
	...
	ulint		latch_mode;	/*!< see TODO note below!
					...
					stored in old_rec ,同函数btr_cur_search_to_nth_level中的参数*/
	ulint		old_stored;	/*!< BTR_PCUR_OLD_STORED
					or BTR_PCUR_OLD_NOT_STORED ，表示是否有记录放在持久游标中 */
	rec_t*		old_rec;	/*!< if cursor position is stored,
					contains an initial segment of the
					latest record cursor was positioned
					either on, before, or after，持久游标保存查询得到的记录 */
	ulint		old_n_fields;	/*!< number of fields in old_rec */
	ulint		rel_pos;	/*!< BTR_PCUR_ON, BTR_PCUR_BEFORE, or
					BTR_PCUR_AFTER, depending on whether
					cursor was on, before, or after the
					old_rec record，持久游标查询得到的位置，
					有效值为：BTR_PCUR_ON 定位到用户记录，old_rec保存指向用户的记录。
					        BTR_PCUR_BEFORE定位到infimum记录，old_rec保存第一个用户记录。
							BTR_PCUR_AFTER定位到supermum,old_rec保存最后一个用户记录*/
	buf_block_t*	block_when_stored;/* buffer block when the position was
					stored */
	ib_uint64_t	modify_clock;	/*!< the modify clock value of the
					buffer block when the cursor position
					was stored,上次保存的记录信息是页的clock信息，用于判断
					是否在restore过程中页发生变化，若有则需要重新定位记录 */
	enum pcur_pos_t	pos_state;	/*!< btr_pcur_store_position() and
					btr_pcur_restore_position() state. 
					  标记位置信息 */
	ulint		search_mode;	/*!< PAGE_CUR_G, ... 同函数btr_cur_search_to_nth_level中的参数*/
