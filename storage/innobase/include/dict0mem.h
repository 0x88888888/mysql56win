...
struct dict_index_t{
	...
	unsigned	page:32;/*!< root页的page_no,
						     index tree root page number */
#endif /* !UNIV_HOTBACKUP */
	unsigned	type:DICT_IT_BITS; // DICT_IT_BITS == 6
				/*!< 
				索引类型：聚集索引，唯一索引，普通索引，insert buffer索引，已经corrupt的索引
...
# error (1<<MAX_KEY_LENGTH_BITS) < MAX_KEY_LENGTH
	...
	unsigned	n_nullable:10;/*!< 可空列的数量， number of nullable fields */
	...
	btr_search_t*	search_info;
				/*!< info used in optimistic searches ,保存索引的访问模式，根据这个信息建立自适应哈希*/
...
struct dict_table_t{
	...
	unsigned	n_cols:10;/*!< 表中所有的列的数量，number of columns */
	...
	dict_col_t*	cols;	/*!< 列数组 array of column descriptions */
...
#define BG_STAT_SHOULD_QUIT	(1 << 1)
	...
	const trx_t*	autoinc_trx;//拥有autoinc lock的事物
