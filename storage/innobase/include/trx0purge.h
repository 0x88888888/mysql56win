...
/** The control structure used in the purge operation
   purge_sys用于保存当前purge的位置与信息
*/
struct trx_purge_t{
	sess_t*		sess;		/*!< System session running the purge query */

	trx_t*		trx;		/*!< purge操作同样被视为一个事务，
							     只是该事务的类型TRX_PURGE
							  System transaction running the purge query: 
							  this trx is not in the trx list of the trx system and it
                              never ends */
	rw_lock_t	latch;		/*!< 
							变量latch用于保证删除undo日志的正确性，保证删除日志时没有其他事物正在引用该undo日志。
							当开启purge操作时，首先需要持有该x-latch，然后再通过函数read_veiw_oldest_copy_or_open_new用以
							判断哪些undo日志可以被清理(注意purge是一个特殊的事务，事务类为TRX_PURGE，其他都是用户事务)。
							当用户需要通过undo日志进行多版本并发控制，即非锁定的一致性读时，其需要首先获得该对象的s-latch
	...
	volatile purge_state_t	state;	/*!< 当前purge的操作的状态，
									     Purge coordinator thread states,
...
struct trx_purge_rec_t {
	...
	*/
	...
	/*
	   typedef ib_uint64_t ib_id
	   typedef ib_id roll_ptr_t
	*/
