...
/* @{ */
/* 
使用2个bit来描述page的空闲空间范围：
          0（0 bytes）、1（512 bytes）、2（1024 bytes）、3（2048 bytes）
*/
...
/******************************************************************//**
获取page number = 3 的 insert buffer header page的 frame
...
/******************************************************************//**
得到FSP_IBUF_TREE_ROOT_PAGE_NO对应的frame
...
/******************************************************************//**
初始化 insert buffer 数据结构
包括，ibuf对象、ibuf_pessimistic_insert_mutex_key
ibuf_mutex_key、ibuf_bitmap_mutex_key、ibuf_bitmap_mutex_key
对象
...
/*=======================*/
{
	...
	/* 得到 IBUF_HEADER_PAGE_NO对应的 page 的frame */
	header_page = ibuf_header_page_get(&mtr);

	                     /*header_page + 94 + 0*/
	fseg_n_reserved_pages(header_page + IBUF_HEADER + IBUF_TREE_SEG_HEADER,
			      &n_used /* n_used 为对应inode已经被使用的page数量*/, 
				  &mtr);
...

	{
		...
		/* 得到4号页对应的 page block */
		block = buf_page_get(
			IBUF_SPACE_ID, 0/*zip_size*/, 
			FSP_IBUF_TREE_ROOT_PAGE_NO /* page number */,
			RW_X_LATCH, &mtr);
...
# ifdef UNIV_DEBUG
/** 
获取ibuf ibuf_bitmap中的表示空闲大小的2个bits
根据bits获取相应的值
...
/********************************************************************//**
Gets the desired bits for a given page from a bitmap page.
根据page_no，bit，从insert buffer bitmap中得到相应的bits
@return	value of bits */
UNIV_INLINE
ulint
...
# error "IBUF_BITS_PER_PAGE % 2 != 0"
	...
	byte_offset = bit_offset / 8;//字节
	bit_offset = bit_offset % 8; //字节内的位偏移
...
/********************************************************************//**
更具page_no设置insert buffer bitmap相应的bits
...
计算page_no所对应的insert buffer bitmap页的page_no，
每隔16000多个page，就会有一个insert buffer bitmap page
...
/***********************************************************************//**
确认是否是insert buffer 的level 2 或者 level 3的 page
...
/* 用于将保存在insert buffer中的记录转化为待插入到实际辅助索引页的逻辑记录 */
#ifdef UNIV_DEBUG
...
its fields.
在插入insert buffer 之前，需要将数据重构，以page_no为健值
	...
	mem_heap_t*	heap)	/*!< in: heap into which to build */
{
	...
	/* 将entry中的fileds逐个填写到 tuple的field中去 */
	for (i = 0; i < n_fields; i++) {
		ulint			fixed_len;
...
构造在insert buffer索引树种查询指定辅助索引页的记录 
...
/*********************************************************************//**
添加一个空闲page到ibuf root page的PAGE_BTR_IBUF_FREE_LIST中去
...
/*====================*/
{
	...
	/* 标记第IBUF_BITMAP_IBUF位为true,表示该page为IBUF TREE的节点 */
	ibuf_bitmap_page_set_bits(
...
/*********************************************************************//**
将insert buffer 中的 free page归还到buffer pool 中去
...
/*=======================*/
{
    ...
    /* 获取page number = 3 的 insert buffer page的 frame */
	header_page = ibuf_header_page_get(&mtr);
	...
	*/
	...
	/* 得到insert buffer的 free list 的首个page */
	...
	/*
	  获取到FSP_IBUF_TREE_ROOT_PAGE_NO对应的frame
	*/
	root = ibuf_tree_root_get(&mtr);
...
/***********************************************************************//**
有时候，insert buffer 会有很多的空闲的page，
释放insert buffer中的空闲的page
...
ibuf_insert_low(
/*============*/
	ulint		mode,	/*!< in: BTR_MODIFY_PREV(乐观) or BTR_MODIFY_TREE(悲观) */
	ibuf_op_t	op,	/*!< in: operation type */
	ibool		no_counter,
...
check_watch:
...

	{
		...
		/* 如果当前要进行插入的目标page，正在被pruging，那就返回false */
...
/*********************************************************************//**
将insert buffer缓存中的记录插入的到辅助索引中给你，不论主动还是被动合并，
最终都会调用该函数进行记录的合并操作，记录合并完成后，删除insert buffer中的记录。
