                           ...
                           uint *key_count, int select_field_count)
{
  ...
  for (field_no=0; (sql_field=it++) ; field_no++)
  {
    ...
    */
    sql_field->length= sql_field->char_length;//字符数量
    /* Set field charset. */
  ...
  while ((sql_field=it++))
  {
    ...
    sql_field->offset= record_offset;//确定列在行中的偏移
  ...
  *key_count= 0;
  ...
  while ((key=key_iterator++))
  {
    ...
    key_iterator2.rewind (); //重新到fisrt element 
    if (key->type != Key::FOREIGN_KEY)
    {
    ...
    if (key->name.str != ignore_key) //不过滤掉
          ...
          if (key->type == Key::SPATIAL)
          {
                ...
                (f_is_geom(sql_field->pack_flag) && key->type != Key::SPATIAL))
	       {
	        ...
	        if (!(sql_field->flags & NOT_NULL_FLAG))
	        {
               ...
               {   //非主键
                   key_info->flags|= HA_NULL_PART_KEY;// 索引可以为null
