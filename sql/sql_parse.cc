...
mysql_execute_command(THD *thd)
{
  ...
  */
  ...
  /*
	...
	thd上的老的请求，或许需要rollback，在下一个事务开始之前，老的语句如果有rollback，
	那么这个rollback需要已经结束了
  */
  ...
  */
  ...
  else
  {
    ...
    /*
	  ...
	  slave上 readonly的情况，不能执行update语句
    */
...
case SQLCOM_DBXP_SELECT:
  {
	  ...
	  const Name_string s("Id",2);
	  field_list.push_back(new Item_int(s, (long long) 1, 21));
...
mysql_new_select(LEX *lex, bool move_down)
{
  ...
  if (move_down)
  {
	...
	//unit->prev=lex->current_select->slave
	...
	/* 到此，构建关系
	                     <---master---    <---master---
	  lex->current_select             unit              select_lex 
	                      ---slave--->    ---slave---> 
  ...
  else
  {
	...
	/*
	 lex->current_select->next = select_lex
	 lex->current_select->next->next = select_lex
	 select_lex->prev = lex->current_select
	 select_lex->master = lex->current_select ->master
	*/
  ...
  if (query_cache_send_result_to_client(thd, rawbuf, length) <= 0)
  {
    ...
    if (!err)
    {
      /*
		...
		重写sql语句，记录到日志的时候，掩盖掉sql中的明文密码之类的
      */
                                             ...
                                             LEX_STRING *option)
{
  ...
  if (lock_type != TL_IGNORE)
  {
	...
	//当前的table不能在 SELECT_LEX中出现过相同的名称
  ...
  */
  ...
  lex->add_to_query_tables(ptr);//添加到thd->lex->query_tables_last中去
