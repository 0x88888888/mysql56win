/*
   ...
   DESCRIPTION
     this file contains the Query_tree class. It is responsible for contianing the
	 ...
	 a binary "compiled" for of the query.
...
public:
   ...
   /*
	  ...
	  DESCRIPTION 
	     This is structure contains all of the data for a query node:

		 NodeId -- the internal id number for a node
		 ParentNodeId -- the internal id for parent node (used for insert)
    ...
    */
	...
	public:
		...
		void print_join(int space);
		void print_restrict(int space);
		void print_projection(int space);
		void print_distinct(int space);
		void print_sort(int space);
...
private:
	...
	record_buff *left_record_buff_ptr;
	record_buff *right_record_buff_ptr;
