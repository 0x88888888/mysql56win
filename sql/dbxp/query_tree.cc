...
#define START_SPACE    4

void Query_tree::print(char* tag) {
	printf(tag);
	this->root->print(START_SPACE);
}



void Query_tree::query_node::print(int space){
	switch (this->node_type)
	{
	   case Query_tree::query_node_type::qntJoin:
		   this->print_join(space);
		   break;
	   case Query_tree::query_node_type::qntRestrict:
		   this->print_restrict(space);
		   break;
	}

	if(this->left){
		this->left->print(space+4);
	}
	if(this->right){
		this->right->print(space+4);
	}
}

void Query_tree::query_node::print_join(int space){
	if(this->node_type ==Query_tree::query_node_type::qntJoin){
		print_space(space);
		printf("node_type : qntJoin , join_type=");
		switch (this->join_type)
		{
		case type_join::jnUNKNOWN:
            printf("    jnUNKNOWN");
			break;
		case type_join::jnFULLOUTER:
			printf("    jnFULLOUTER");
			break;
		case type_join::jnINNER:
			printf("    jnINNER");
			break;
		case type_join::jnINTERSECT:
			printf("    jnINTERSECT");
			break;
		case type_join::jnLEFTOUTER:
			printf("    jnLEFTOUTER");
			break;
		case type_join::jnRIGHTOUTER:
			printf("    jnRIGHTOUTER");
			break;
		case type_join::jnUNION:
			printf("    jnUNION");
			break;
		case type_join::jnCROSSPRODUCT:
			printf("    jnCROSSPRODUCT");
			break;
		default:
			break;
		}
		...
		printf("join_cond=");
		switch (this->join_cond)
		{
		case join_con_type::jcNA:
			printf("jcNA");
			break;
		case join_con_type::jcON:
			printf("jcON");
			break;
		case join_con_type::jcUN:
			printf("jcUN");
			break;
		case join_con_type::jcUS:
			printf("jcUS");
			break;
		default:
			printf("qntJoin node, but there is error on join_cond");
			exit(0);
			break;
		}
		printf("\n");
		Expression *join_expr= this->join_expr;
		int i=join_expr->num_expressions();
		for(int j=0; j<i; j++){
			expr_node *exp_node= join_expr->get_expression(j);
			//exp_node->junction->
			//exp_node->
		}
	}
	...
	//if(this->join_type != Query_tree::type_join::)
}

void Query_tree::query_node::print_restrict(int space) {
	if(this->node_type ==Query_tree::query_node_type::qntRestrict){
		Expression* where_expr= this->where_expr;
		for(int i=0; i<where_expr->num_expressions(); i++){

			expr_node* node= where_expr->get_expression(i);
    ...
    */
       ...
       ((cur_node->left == 0) || (cur_node->right == 0)))
    {
      ...
      */
      if(prev == 0) 
      {
         if(cur_node->left == 0 && (cur_node->right!= NULL) )
        {
...
{//如果一个node既有where又有join，那就拆分成两个node
  ...
  if(QN != 0)
  {
       ...
       ((QN->node_type == qntJoin) || (QN->node_type == qntRestrict)))
    {
      ...
      bool isleft = true;//new_node是否作为QN的left child
      /*
        Create a new node and:
      ...
      */
      ...
      query_node *new_node = (query_node*)my_malloc(sizeof(query_node), 
                             MYF(MY_ZEROFILL | MY_WME));
      init_node(new_node);
      new_node->node_type = qntRestrict;//where node
      new_node->parent_nodeid = QN->nodeid;
