...
/*
  ...
  DESCRIPTION
     This method returns the index (numbers) of expression specified.
...
int Expression::index_of(char *table, char *value) {
	...
	while((p != NULL) && (!found)) {
		...
		if ((strcasecmp(((Item_field *)p->left_op)->field_name, value) == 0) &&
			((table == NULL) || 
			(strcasecmp(((Item_field *)p->left_op)->field_name, table) == 0))) {
		   found = true;
...
/*
  ...
  RETURN VALUE
    ...
    Failed = 1
*/
int Expression::convert(THD *thd, Item *mysql_expr /*join_cond*/ ) {
	...
	while (pcond != NULL) {
		...
		if (pcond->type() == Item::COND_ITEM) {
			// join condition item, and、or这些条件表达式
			// 接上，跳过
			enode->junction = pcond;//条件，记录
			pcond = pcond->next;
		} else {
			enode->junction = NULL;
		}

		enode->operation = pcond;//操作符节点
		...
		this->add_expression(true, enode);
...
int Expression::get_join_expr(Expression *where_expr) {
	...
	while(node != NULL) {
		if ( (node->left_op && (node->left_op->type() == Item::FIELD_ITEM)) &&
			(node->right_op && (node->right_op->type() == Item::FIELD_ITEM)) ) {
				//如果同时又左右节点，并且左右节点都是FIELD类型的，那就是join条件了
