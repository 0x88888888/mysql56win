		      ...
		      handler *db_file)
{
  ...
  //确定info_length这个值，是只在screen_buff中填过数据的长度
  ...
  //每一行的数据的开始位置之前几个null_bits位置
  //这个create_info->null_bits =null_fields 在 mysql_prepare_create_table阶段设置;
  ...
  */
  ...
  format_section_length= format_section_header_size + 
	                     tablespace_length + 1 +
  ...
  */
  ...
  //写入fileinfo，然后写入keybuff
  if (mysql_file_pwrite(file, fileinfo, 64, 0L, MYF_RW) ||
      mysql_file_pwrite(file, keybuff, key_info_length, //在uint2korr(fileinfo+6)出写入长度为key_info_length的keybuff
                        (ulong) uint2korr(fileinfo+6), MYF_RW))
    goto err;

  //跳过 fileinfo和keybuff数据，继续写
  mysql_file_seek(file,
                  (ulong) uint2korr(fileinfo+6) + (ulong) key_buff_length,
                  MY_SEEK_SET, MYF(0));

  //创建一个默认值的行，写入file中去,位置： uint2korr(fileinfo+6) + (ulong) key_buff_length 
  ...
  /* "Format section" with additional table and column properties */
  {
    ...
    *ptr= 0; /* tablespace string terminating zero */
    ...
    while ((field=it++))
    {
      ...
      const uchar field_flags= field_storage + (field_column_format << COLUMN_FORMAT_SHIFT);
  ...
  //写入forminfo和screen_buff,  create_fields 信息
                           ...
                           bool small_file)
{
  ...
  DBUG_ENTER("pack_screens");
                                    //  fields_on_screen = 22+1 -4 =19
  start_row=4; end_row=22; cols=80; fields_on_screen=end_row+1-start_row;

  *screens=(fields-1)/fields_on_screen+1; //最小值是1, 20个elements 1个screen

  //       1 *(4 + ( 80>>1 ) +4) == 1 * (4 + 40 + 4) =49
  length= (*screens) * (SC_INFO_LENGTH+ (cols>> 1)+4);

  
  Create_field *field;
  while ((field=it++))                         // 1+ 3 + 40
