...
public:
  ...
  */
  ...
  /*
	...
	fake_select_lex用于存储 union 查询的order by, limit 条件
  */
...
public:
  ...
  */
  ...
  //下面的两个join和上面的join不是一回事情
  List<TABLE_LIST> top_join_list; /* join list of the top level          */
  List<TABLE_LIST> *join_list;    /* list for the currently parsed join  */

  //embedding 表示当前正在处理的nest table
  ...
  */
  ...
  uint in_sum_expr;
  uint select_number; /* number of select (used for EXPLAIN)， explain中的id */
  /**
...
public:
  ...
  /**
	 ...
	 在优化(比如in2exists，未必是优化，称转写比较合适)失败的时候，设置这个m_broken标记
