                           ...
                           File file)
{
  ...
  mysql_file_seek(file,pos,MY_SEEK_SET,MYF(0)); //跳过pos个长度
  ...
  */
  ...
  //读取 head+ 6 处数据指定的位置的 key_info_length个字节
  ...
  //只有innodb支持HTON_SUPPORTS_EXTENDED_KEYS
  use_extended_sk=
  ...
  n_length= uint2korr(forminfo+268); // 2 + N个(strlen(field_name) + 1) 
  ...
  share->field= field_ptr;
   //所有fields的固定空间 +screens的空间 + 所有fields的name空间  + interval的空间 + 各个fields的注释空间
